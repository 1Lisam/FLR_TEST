(() => {
  ...
  var require_a_scene_adapter = __commonJS({
    "prototype/hybrid_v48/a_scene_adapter.js"(exports, module) {
      ...
      function canonicalFromA(inspect2, options = {}) {
        const mapping = options.scenePlayerIdMapping;
        if (!mapping) throw new Error("A_PLAYER_MAPPING_REQUIRED_FOR_RETURN");
        if (!Array.isArray(inspect2.players) || inspect2.players.length !== 22 || new Set(inspect2.players.map((p) => p.id)).size !== 22) throw new Error("A_RETURN_REQUIRES_22_UNIQUE_PLAYERS");
        for (const p of inspect2.players) mapping.canonicalForA(p.id);
        if (inspect2.ball.owner != null) mapping.canonicalForA(inspect2.ball.owner);
        return Canonical.create({ tick: inspect2.tick, clock: inspect2.clock, phase: inspect2.phase, score: inspect2.score, possessionTeam: inspect2.possession, players: inspect2.players.map((p) => ({ id: mapping.canonicalForA(p.id), team: p.team, role: p.role, x: p.x, y: p.y, held: Math.max(0, inspect2.time - p.controlSince) })), ball: { x: inspect2.ball.x, y: inspect2.ball.y, z: inspect2.ball.z, ownerId: inspect2.ball.owner == null ? null : mapping.canonicalForA(inspect2.ball.owner), mode: inspect2.ball.mode }, metadata: { aVersion: inspect2.version } });
      }
      function inspectInBounds(inspect2) {
        const b = inspect2 && inspect2.ball;
        if (!b || !Number.isFinite(b.x) || !Number.isFinite(b.y) || b.x < 0 || b.x > 105 || b.y < 0 || b.y > 68) return false;
        return Array.isArray(inspect2.players) && inspect2.players.every((p) => Number.isFinite(p.x) && Number.isFinite(p.y) && p.x >= 0 && p.x <= 105 && p.y >= 0 && p.y <= 68);
      }
      function terminalBoundaryReady(inspect2, terminal) {
        if (!terminal || !inspectInBounds(inspect2)) return false;
        if (["save", "claim", "block"].includes(terminal.type)) return inspect2.ball.owner !== null && !inspect2.ball.flight;
        return inspect2.phase !== "play" || inspect2.ball.owner !== null && !inspect2.ball.flight;
      }
      function begin(canonical, protagonistId, options = {}) {
        ...
        const inspect2 = match.inspect();
        if (!inspectInBounds(inspect2)) return { ok: false, code: "A_BEGIN_OUT_OF_BOUNDS" };
      ...
      function resolve(session, choice, options = {}) {
        ...
        const max = options.maxSteps || 900, observedFrames = [];
        ...
        for (let step = 0; step < max; step++) {
          ...
          if (pendingTerminal && terminalBoundaryReady(currentInspect, pendingTerminal)) return { ok: true, terminal: pendingTerminal, canonicalState: current, steps: step + 1, evidence: session.evidence, observedFrames, match: session.match };
          if (choice.choiceId !== "SHOT" && choice.id !== "SHOT" && currentInspect.phase === "play" && currentInspect.ball.owner !== null && !currentInspect.ball.flight && inspectInBounds(currentInspect) && step >= (options.minimumSteps || 2)) return { ok: true, terminal: { type: "controlledPossession", ownerId: session.mapping.canonicalForA(currentInspect.ball.owner) }, canonicalState: current, steps: step + 1, evidence: session.evidence, observedFrames, match: session.match };
        }
      ...
      module.exports = Object.freeze({ SUPPORTED, TERMINALS, makeSceneState, canonicalFromA, buildScenePlayerIdMapping, begin, inspect, submit, resolve, run, actionFor, terminalFromEvents, inspectInBounds });
    }
  });
  ...
  var require_hybrid_match_controller = __commonJS({
    "prototype/hybrid_v48/hybrid_match_controller.js"(exports, module) {
      ...
      var HybridMatchController = class {
        constructor({ donor, donorContract = {}, aEngine, aOptions = {}, protagonistId, replayLimit = 180 }) {
        ...
        advanceMacro() {
          ...
          if (trigger) {
            this.donor.pause();
            const session = AScene.begin(canonical, this.protagonistId, { engine: this.aEngine, ...this.aOptions });
            ...
            const candidates = Choices.generate(canonical, this.protagonistId);
            this.pending = { canonical, trigger, candidates, session, shownMatch: session.match };
            this.state = STATES.INTERACTIVE_PENDING;
            return { ok: true, interactive: true, candidates, aInspect: shown, aMatch: session.match, hostClock: this.hostClockStatus() };
          }
          return { ok: true, interactive: false, hostClock: this.hostClockStatus() };
        }
        submitAction({ choiceId, targetId = null, aim = null, playerId: playerId2 = this.protagonistId }) {
          ...
          const choice = { choiceId, targetId, playerId: playerId2, aim: aim || (choiceId === "SHOT" ? { x: 105, y: 34 } : choiceId === "CARRY" ? { x: Math.min(105, this.pending.canonical.players.find((p) => p.id === playerId2).x + 8), y: this.pending.canonical.players.find((p) => p.id === playerId2).y } : null) };
          ...
          this.donor.resume();
          this.state = STATES.MACRO_RUNNING;
          return { ok: true, terminal: scene.terminal, state: this.state, replayAvailable: this.replay.captures.length > 0, sceneFrames: scene.observedFrames || [], evidence: scene.evidence };
        }
        fail(code, detail = {}) {
          this.error = code;
    ...
    "frl_a_validation/engine_b_candidate.js"(exports, module) {
      (function(root, factory) {
        ...
        function launch(s, p, type, target, loft = 0) {
          ...
          const speed2 = type === "shot" ? 23 + p.attributes.shooting * 8 : clamp(10 + length * 0.36, 12, 23);
          b.vx = dx / Math.max(0.1, length) * speed2;
          b.vy = dy / Math.max(0.1, length) * speed2;
          b.vz = loft;
          b.owner = null;
          b.mode = "flight";
          b.lastTouch = p.id;
        ...
        function touchFreeBall(s, old) {
          ...
          if (h.keeper) {
            ...
            const difficulty = speed2 / 34 + h.distance / 3 + b.z / 7;
            ...
            if (clean) {
              ...
              b.vx = normal.x / n * Math.max(4, speed2 * 0.45);
              b.vy = normal.y / n * Math.max(4, speed2 * 0.45) + (random(s) - 0.5) * 3;
              b.vz = 1.5;
              b.mode = "loose";
              b.owner = null;
              b.lastTouch = p.id;
            ...
            const control = p.attributes.control + (intendedReceipt ? 0.58 : 0.46) - speed2 / 70 - b.z * 0.15 - pressurePenalty;
        ...
        function integrateBall(s) {
          ...
          if (b.owner !== null) {
            const p = s.players[b.owner], speed2 = Math.hypot(p.vx, p.vy);
            const old2 = { x: b.x, y: b.y, z: b.z };
            b.x = p.x + (speed2 > 0.4 ? p.vx / speed2 : direction(s, p.team)) * 0.55;
            b.y = p.y + (speed2 > 0.4 ? p.vy / speed2 : 0) * 0.55;
            b.z = 0.15;
            boundary(s, old2);
            return;
          }
        ...
        const _Match = class _Match {
          ...
          advance() {
            return __privateGet(this, _s).paused ? false : tick2(__privateGet(this, _s));
          }
          step() {
            return __privateGet(this, _s).paused ? tick2(__privateGet(this, _s)) : false;
          }
          get paused() {
  ...
  // ../../../../vendor_cache/footballsim-ea35bbd3245af9c822f7db5cb359b119fb60ab33/dist/lib/actions/intent.js
  ...
  // ../../../../vendor_cache/footballsim-ea35bbd3245af9c822f7db5cb359b119fb60ab33/dist/lib/logger.js
  var logger = {
    info: (...args) => {
      if (false)
  ...
  // ../../../../vendor_cache/footballsim-ea35bbd3245af9c822f7db5cb359b119fb60ab33/dist/lib/common/getBallTrajectory.js
  ...
  // ../../../../vendor_cache/footballsim-ea35bbd3245af9c822f7db5cb359b119fb60ab33/dist/lib/common.js
  ...
  // ../../../../vendor_cache/footballsim-ea35bbd3245af9c822f7db5cb359b119fb60ab33/dist/lib/playerDefaults.js
  ...
  // ../../../../vendor_cache/footballsim-ea35bbd3245af9c822f7db5cb359b119fb60ab33/dist/lib/factories/playerFactory.js
  ...
  // ../../../../vendor_cache/footballsim-ea35bbd3245af9c822f7db5cb359b119fb60ab33/dist/lib/setVariables.js
  ...
  // ../../../../vendor_cache/footballsim-ea35bbd3245af9c822f7db5cb359b119fb60ab33/dist/lib/event/goal.js
  ...
  // ../../../../vendor_cache/footballsim-ea35bbd3245af9c822f7db5cb359b119fb60ab33/dist/lib/position/set-pieces/corners.js
  ...
  // ../../../../vendor_cache/footballsim-ea35bbd3245af9c822f7db5cb359b119fb60ab33/dist/lib/position/set-pieces/restarts.js
  ...
  // ../../../../vendor_cache/footballsim-ea35bbd3245af9c822f7db5cb359b119fb60ab33/dist/lib/boundaryHandler.js
  ...
  // ../../../../vendor_cache/footballsim-ea35bbd3245af9c822f7db5cb359b119fb60ab33/dist/lib/position/freekick.js
  ...
  // ../../../../vendor_cache/footballsim-ea35bbd3245af9c822f7db5cb359b119fb60ab33/dist/lib/setFreekicks.js
  ...
  // ../../../../vendor_cache/footballsim-ea35bbd3245af9c822f7db5cb359b119fb60ab33/dist/lib/stats.js
  ...
  // ../../../../vendor_cache/footballsim-ea35bbd3245af9c822f7db5cb359b119fb60ab33/dist/lib/position/penaltyArea.js
  ...
  // ../../../../vendor_cache/footballsim-ea35bbd3245af9c822f7db5cb359b119fb60ab33/dist/lib/setPieces.js
  ...
  // ../../../../vendor_cache/footballsim-ea35bbd3245af9c822f7db5cb359b119fb60ab33/dist/lib/actions/penalty.js
  ...
  // ../../../../vendor_cache/footballsim-ea35bbd3245af9c822f7db5cb359b119fb60ab33/dist/lib/actions/possession.js
  ...
  // ../../../../vendor_cache/footballsim-ea35bbd3245af9c822f7db5cb359b119fb60ab33/dist/lib/position/halftime.js
  ...
  // ../../../../vendor_cache/footballsim-ea35bbd3245af9c822f7db5cb359b119fb60ab33/dist/lib/setPositions.js
  ...
  // ../../../../vendor_cache/footballsim-ea35bbd3245af9c822f7db5cb359b119fb60ab33/dist/lib/setBottomFreekicks.js
  ...
  // ../../../../vendor_cache/footballsim-ea35bbd3245af9c822f7db5cb359b119fb60ab33/dist/lib/setTopFreekicks.js
  ...
  // ../../../../vendor_cache/footballsim-ea35bbd3245af9c822f7db5cb359b119fb60ab33/dist/lib/position/set-pieces/penalties.js
  ...
  // ../../../../vendor_cache/footballsim-ea35bbd3245af9c822f7db5cb359b119fb60ab33/dist/lib/position/proximity.js
  ...
  // ../../../../vendor_cache/footballsim-ea35bbd3245af9c822f7db5cb359b119fb60ab33/dist/lib/utils/assert.js
  ...
  // ../../../../vendor_cache/footballsim-ea35bbd3245af9c822f7db5cb359b119fb60ab33/dist/lib/ai/threatAnalysis.js
  ...
  // ../../../../vendor_cache/footballsim-ea35bbd3245af9c822f7db5cb359b119fb60ab33/dist/lib/intent/utils.js
  ...
  // ../../../../vendor_cache/footballsim-ea35bbd3245af9c822f7db5cb359b119fb60ab33/dist/lib/intent/zones.js
  ...
  // ../../../../vendor_cache/footballsim-ea35bbd3245af9c822f7db5cb359b119fb60ab33/dist/lib/intent/config.js
  ...
  // ../../../../vendor_cache/footballsim-ea35bbd3245af9c822f7db5cb359b119fb60ab33/dist/lib/intent/penaltyBox.js
  ...
  // ../../../../vendor_cache/footballsim-ea35bbd3245af9c822f7db5cb359b119fb60ab33/dist/lib/intentLogic.js
  ...
  // ../../../../vendor_cache/footballsim-ea35bbd3245af9c822f7db5cb359b119fb60ab33/dist/lib/playerSelectors.js
  ...
  // ../../../../vendor_cache/footballsim-ea35bbd3245af9c822f7db5cb359b119fb60ab33/dist/lib/actions/findPossActions.js
  ...
  // ../../../../vendor_cache/footballsim-ea35bbd3245af9c822f7db5cb359b119fb60ab33/dist/lib/actions/tackle.js
  ...
  // ../../../../vendor_cache/footballsim-ea35bbd3245af9c822f7db5cb359b119fb60ab33/dist/lib/actions/booking.js
  ...
  // ../../../../vendor_cache/footballsim-ea35bbd3245af9c822f7db5cb359b119fb60ab33/dist/lib/validation/action.js
  ...
  // ../../../../vendor_cache/footballsim-ea35bbd3245af9c822f7db5cb359b119fb60ab33/dist/lib/actions.js
  ...
  // ../../../../vendor_cache/footballsim-ea35bbd3245af9c822f7db5cb359b119fb60ab33/dist/lib/injury.js
  ...
  // ../../../../vendor_cache/footballsim-ea35bbd3245af9c822f7db5cb359b119fb60ab33/dist/lib/actions/defensiveActions.js
  ...
  // ../../../../vendor_cache/footballsim-ea35bbd3245af9c822f7db5cb359b119fb60ab33/dist/lib/actions/ballTrajectory.js
  ...
  // ../../../../vendor_cache/footballsim-ea35bbd3245af9c822f7db5cb359b119fb60ab33/dist/lib/kickLogic.js
  ...
  // ../../../../vendor_cache/footballsim-ea35bbd3245af9c822f7db5cb359b119fb60ab33/dist/lib/actions/triggers.js
  ...
  // ../../../../vendor_cache/footballsim-ea35bbd3245af9c822f7db5cb359b119fb60ab33/dist/lib/physics.js
  ...
  // ../../../../vendor_cache/footballsim-ea35bbd3245af9c822f7db5cb359b119fb60ab33/dist/lib/position/trajectory.js
  var mockPlayer = {
    name: "George Johnson",
    shirtNumber: 45,
    position: "ST",
  ...
  // ../../../../vendor_cache/footballsim-ea35bbd3245af9c822f7db5cb359b119fb60ab33/dist/lib/teamAi.js
  ...
  // ../../../../vendor_cache/footballsim-ea35bbd3245af9c822f7db5cb359b119fb60ab33/dist/lib/position/movement.js
  ...
  // ../../../../vendor_cache/footballsim-ea35bbd3245af9c822f7db5cb359b119fb60ab33/dist/lib/position/ball.js
  ...
  // ../../../../vendor_cache/footballsim-ea35bbd3245af9c822f7db5cb359b119fb60ab33/dist/lib/actions/ball.js
  ...
  // ../../../../vendor_cache/footballsim-ea35bbd3245af9c822f7db5cb359b119fb60ab33/dist/lib/position/offside.js
  ...
  // ../../../../vendor_cache/footballsim-ea35bbd3245af9c822f7db5cb359b119fb60ab33/dist/lib/playerMovement.js
  ...
  // ../../../../vendor_cache/footballsim-ea35bbd3245af9c822f7db5cb359b119fb60ab33/dist/lib/ballActionHandler.js
  ...
  // ../../../../vendor_cache/footballsim-ea35bbd3245af9c822f7db5cb359b119fb60ab33/dist/lib/actions/deflections.js
  ...
  // ../../../../vendor_cache/footballsim-ea35bbd3245af9c822f7db5cb359b119fb60ab33/dist/lib/collisions.js
  ...
  // ../../../../vendor_cache/footballsim-ea35bbd3245af9c822f7db5cb359b119fb60ab33/dist/lib/ballMovement.js
  ...
  // ../../../../vendor_cache/footballsim-ea35bbd3245af9c822f7db5cb359b119fb60ab33/dist/lib/ballState.js
  ...
  // ../../../../vendor_cache/footballsim-ea35bbd3245af9c822f7db5cb359b119fb60ab33/dist/lib/validate.js
  ...
  // ../../../../vendor_cache/footballsim-ea35bbd3245af9c822f7db5cb359b119fb60ab33/dist/engine.js
  ...
  // ../../../../vendor_cache/footballsim-ea35bbd3245af9c822f7db5cb359b119fb60ab33/src/init_config/team1.json
  var team1_default = {
    name: "ThisTeam",
    rating: 88,
    players: [
  ...
  // ../../../../vendor_cache/footballsim-ea35bbd3245af9c822f7db5cb359b119fb60ab33/src/init_config/team2.json
  var team2_default = {
    name: "ThatTeam",
    rating: 88,
    players: [
  ...
  // ../../../../vendor_cache/footballsim-ea35bbd3245af9c822f7db5cb359b119fb60ab33/src/init_config/pitch.json
  var pitch_default = {
    pitchWidth: 680,
    pitchHeight: 1050,
    goalWidth: 90
  ...
  // deploy/v48/.v48-fullmatch-entry.generated.mjs
  ...
  var FLR = "c6087b73aaa094feea0e1afea985c5846619f48f";
  ...
  var playing = true;
  var speed = 1;
  var PHASE_KO = { MACRO_RUNNING: "저해상도 경기 진행", INTERACTIVE_PENDING: "A엔진 선택 장면", DECISION_PAUSED: "선택 대기", REPLAY_PAST: "이전 장면 재생", A_SCENE: "선택 장면 재생", DONOR_RESUMED: "경기 재개", FULL_TIME: "경기 종료", FAILED_CLOSED: "안전 중단" };
  ...
  function draw(s) {
    ...
    ctx.strokeStyle = "#d9f4da";
    ctx.strokeRect(10, 10, 820, 524);
    ctx.beginPath();
    ctx.moveTo(420, 10);
    ctx.lineTo(420, 534);
    ctx.stroke();
    for (const p of s.players) {
      ctx.beginPath();
      ctx.fillStyle = p.team === 0 ? "#f0c34b" : "#ed6970";
      ctx.arc(10 + p.x / 105 * 820, 10 + p.y / 68 * 524, 8, 0, 7);
      ctx.fill();
    }
    ctx.beginPath();
    ctx.fillStyle = "white";
    ctx.arc(10 + s.ball.x / 105 * 820, 10 + s.ball.y / 68 * 524, 5, 0, 7);
    ctx.fill();
  }
  ...
  function formatHostClock(seconds) {
    return `${String(Math.floor(seconds / 60)).padStart(2, "0")}:${String(seconds % 60).padStart(2, "0")}`;
  }
  function render() {
    const s = displayState(), pending = harness.pending, clock = harness.hostClockStatus();
    draw(pending?.canonical || s);
    $("phase").textContent = `${PHASE_KO[harness.state] || harness.state} · ${formatHostClock(clock.seconds)}`;
    $("choices").replaceChildren(...harness.state === import_hybrid_v48.default.STATES.FULL_TIME ? [] : (pending?.candidates || []).filter((c) => ["PASS", "PROGRESSIVE_PASS", "THROUGH_PASS", "SAFE_PASS", "CARRY", "SHOT"].includes(c.id)).map((c) => {
    ...
    $("diag").textContent = JSON.stringify({ "테스트 전용": true, "엔진 모드": "실제 엔진 / 브라우저 내장", "V48 Hybrid host match clock": `${formatHostClock(clock.seconds)} (도너 네이티브 시간이 아님)`, "하프타임 경계": clock.halfTimeTransitions, "풀타임 경계": clock.fullTimeTransitions, "최종 스코어": harness.state === import_hybrid_v48.default.STATES.FULL_TIME ? s.score : null, "handoff 수": harness.handoffCount || 0, "FLR 기준 커밋": FLR, "도너 SHA": SHA, "주인공 ID": harness.protagonistId, "선택 ID": cid ? `${CHOICE_KO[cid] || cid} (${cid})` : null, "선택 대상 보존": "내부 choiceId + targetId 정확히 유지", "현재 상태만 사용": true, "미래 선계산 없음": true, "이전 장면 프레임 수": harness.replay.current().length, "A 모드 렌더 소스": "retained A Match.inspect()", "화면 선수 수": s.players.length, "화면 안 선수 수": s.players.filter((p) => p.x >= 0 && p.x <= 105 && p.y >= 0 && p.y <= 68).length, "좌표축": "도너 [폭,길이] → FLR x=길이, y=폭", "동일 도너 경기 객체 유지": true, "하프 전환 제한": "도너 방향/전술/선수 재배치 없음", "오프사이드 처리": "단독 오프사이드는 안전 중단 처리", "종료 이벤트": harness.lastScene?.terminal?.type ?? null, "오류": harness.error ?? null }, null, 2);
  }
  function tick() {
    if (playing && harness.state === import_hybrid_v48.default.STATES.MACRO_RUNNING) {
      for (let i = 0; i < speed && harness.state === import_hybrid_v48.default.STATES.MACRO_RUNNING; i++) harness.advanceMacro();
    }
    render();
  }
  $("seed").value = "496001";
