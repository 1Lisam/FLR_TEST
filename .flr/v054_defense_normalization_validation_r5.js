...
// Mirror wide ownership: left wing must belong to same-side A-RB just as right wing belongs to A-LB.
{
...
const status=(base.returncode===0&&checks.every(x=>x.pass))?'PASS':'FAIL';console.log(JSON.stringify({schemaVersion:'FLR_V054_DEFENCE_NORMALIZATION_VALIDATION_R5',status,baseStatus:obj.status,checks,watches},null,2));if(status!=='PASS')process.exitCode=1;
