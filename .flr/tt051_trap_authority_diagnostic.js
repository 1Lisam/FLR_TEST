...
function runCase(explicit){
  ...
  for(const p of m.players){if(p.team==='HOME')setP(p,55,8+(p.slot?.length||1)*4);else setP(p,88,8+(p.slot?.length||1)*4);}
  ...
  for(let i=0;i<80&&!s.pending;i++)P.step(s,.05);
  const pending=s.pending;const trap=pending?.options?.find(o=>o.id==='TRAP_CONTROL');
  if(!trap)return{explicit,valid:false,reason:'NO_TRAP_PENDING',time:m.time,pending:pending?{kind:pending.kind,options:pending.options.map(o=>o.id)}:null,events:(m.events||[]).slice(-8)};
  const applied=P.applyChoice(s,'TRAP_CONTROL',null,{source:'QA_TRAP_AUTHORITY'});const chosenAt=m.time;
  let heroReceived=false,receiveAt=null,nextPendingAt=null,autoMaterial=null,lockAtReceive=null,steps=0,offside=false;
  while(!m.completed&&m.time<chosenAt+8&&steps++<300){
    P.step(s,.05);
    if((m.events||[]).some(e=>e.t>chosenAt+.001&&e.type==='OFFSIDE'))offside=true;
  ...
  return{explicit,valid:!offside,applied,chosenAt,heroReceived,receiveAt,nextPendingAt,autoMaterial,offside,lockAtReceive,pending:s.pending?{kind:s.pending.kind,options:s.pending.options.map(o=>[o.id,o.targetId||null])}:null,events:(m.events||[]).filter(e=>e.t>=chosenAt-.01).map(e=>({t:e.t,type:e.type,actorId:e.actorId||null,targetId:e.targetId||null,text:e.text})).slice(-16),futureOutcomePrecomputed:s.futureOutcomePrecomputed};
}
...
const failures=checks.filter(x=>!x.ok);const out={schemaVersion:'FLR_TT051_TRAP_AUTHORITY_DIAG_1.1',pass:failures.length===0,checks,failures,cases:{episode,explicit}};console.log(JSON.stringify(out,null,2));process.exit(failures.length?1:0);
