...
function inspectAttack(m,owner){
  ...
  const cleanShot=!!shot.inBox&&!!shot.openWindow&&(shot.blockers?.length??shot.blockers??99)===0&&(shot.dGoal??99)<=18.5&&Math.abs(pos.y-34)<=12.5&&['ST','WF','CM'].includes(owner.role);
  const runnerLane=pos.x>=80&&!!committed;
  const strong=cleanShot||runnerLane;
  return {strong,cleanShot,runnerLane,pos,shot,committed};
}
function runNpc(seed){
  const m=E.createMatch(seed),out={seed,score:null,strongWindows:0,strongActions:0,harmfulBackpasses:0,deadAttackStalls:0,npcOneTouchPasses:0,examples:[]};
  ...
  while(!m.completed&&guard++<140000){
    ...
    if(watch){
      const actorEvents=newEvents.filter(e=>e.actorId===watch.ownerId&&['PASS','SHOT','TAKE_ON'].includes(e.type));
      const material=actorEvents[0];
      if(material){
        out.strongActions++;
        if(material.type==='PASS'&&['PASS','LONG_PASS'].includes(material.passKind||'PASS')&&material.targetId){
          ...
          if(tx!=null&&tx<fromX-4&&(watch.cleanShot||watch.runnerLane)){
            out.harmfulBackpasses++;
            if(out.examples.length<10)out.examples.push({kind:'HARMFUL_BACKPASS',t:+(material.t??m.time).toFixed(2),owner:watch.ownerId,fromX:+fromX.toFixed(2),target:material.targetId,targetX:+tx.toFixed(2),cleanShot:watch.cleanShot,runnerLane:watch.runnerLane,runnerId:watch.runnerId,runnerLead:watch.runnerLead==null?null:+watch.runnerLead.toFixed(2)});
          }
        }
        watch=null;
      }else{
        const now=ownerOf(m);
        if(!now||now.id!==watch.ownerId||now.controlledSince!==watch.controlledSince){watch=null;}
        else{
          ...
          if(age>=2.2&&move<0.85&&speed<0.75&&!decisionChanged){
            out.deadAttackStalls++;
            if(out.examples.length<10)out.examples.push({kind:'DEAD_ATTACK_STALL',t:+m.time.toFixed(2),owner:watch.ownerId,age:+age.toFixed(2),move:+move.toFixed(2),cleanShot:watch.cleanShot,runnerLane:watch.runnerLane,action:now.action,lastDecision:now.lastDecision,nextThink:+(now.nextThink||0).toFixed(2)});
            watch=null;
...
function runCarryCadence(seed){
  const s=P.create(seed,{heroPlayerId:'H-ST',mode:'PLAYER_ALL',replaySeconds:10}),out={seed,carries:0,followups:0,badRapidCarryReprompts:0,minDelay:null,examples:[]};
  let watch=null,guard=0;
    ...
    if(s.pending){
      if(watch){
        ...
        if(s.m.ball.ownerId==='H-ST'){
          ...
          if(delay<2.2&&disp<3.0){out.badRapidCarryReprompts++;if(out.examples.length<8)out.examples.push({t:+s.m.time.toFixed(2),delay:+delay.toFixed(2),displacement:+disp.toFixed(2)});}
        }
        watch=null;
      }
      const o=s.pending.options||[],carry=o.find(x=>x.id==='CARRY'),pick=carry||P.autoPick(s)||o[0];
      if(!pick){s.pending=null;continue;}
      const p=s.m.playersById?.['H-ST']||s.m.players.find(x=>x.id==='H-ST');
      const r=P.applyChoice(s,pick.id,pick.targetId||null,{source:'HIST_V04_REFINED_AUDIT'});if(!r.ok){s.pending=null;continue;}
      if(pick.id==='CARRY'){out.carries++;watch={t:s.m.time,x:p?.x||0,y:p?.y||0};}
      continue;
    }
    P.step(s,.1);
    if(watch&&s.m.ball.ownerId!=='H-ST')watch=null;
  }
  if(out.minDelay!=null)out.minDelay=+out.minDelay.toFixed(2);return out;
}
function runLegacy(){
  const script=path.join(ROOT,'.flr/historical_closure_audit.js');if(!fs.existsSync(script))return{status:'MISSING'};
  const tmp=path.join(os.tmpdir(),`flr-historical-${process.pid}.json`);const r=cp.spawnSync(process.execPath,[script,tmp],{cwd:ROOT,encoding:'utf8',maxBuffer:32*1024*1024});
  let obj={status:'FAILED_TO_RUN',exitCode:r.status,stderr:(r.stderr||'').slice(-8000)};try{if(fs.existsSync(tmp))obj=JSON.parse(fs.readFileSync(tmp,'utf8'));}catch(e){obj.parseError=String(e);}try{fs.unlinkSync(tmp);}catch(_e){}return obj;
}
const npc=[];for(let i=1;i<=24;i++)npc.push(runNpc(`HIST-V04-NPC-${i}`));
const carry=[];for(let i=1;i<=8;i++)carry.push(runCarryCadence(`HIST-V04-CARRY-${i}`));
...
const legacyStatus=legacy.status||legacy.result||'UNKNOWN';
const gates={historicalRegression:legacyStatus==='PASS',npcOneTouch:summary.npcOneTouchPasses>0,npcGoodAttackNoDeadStall:summary.deadAttackStalls===0,strongAttackNoHarmfulBackpass:summary.harmfulBackpasses===0,carryCadence:summary.badRapidCarryReprompts===0};
const result={schemaVersion:'FLR_HISTORICAL_CLOSURE_V04_VALIDATION_1.0',summary,gates,status:Object.values(gates).every(Boolean)?'PASS':'FAIL',legacyStatus,npc,carry};
