...
// Role ownership: wide carrier -> same-side FB, not a roaming CM.
{
...
// User/AI CM must not be assigned sticky body marking on a central ST.
{
 ...
 check('CM_NEVER_BODY_MARKS_CENTRAL_ST',cms.every(q=>q.mark!=='H-ST'),cms);
 const st=L('AWAY',m.playersById['H-ST']),cbMarks=['A-LCB','A-RCB'].map(id=>m.playersById[id]).filter(p=>p.markTargetId==='H-ST');
 check('CENTRAL_ST_REMAINS_CB_RESPONSIBILITY',cbMarks.length>=1,{st,cbMarks:cbMarks.map(p=>p.id)});
}

// Deep defence: CMs stay in front of CBs; density inside the box is not itself a failure.
{
...
// Attack support: two 8s reach edge/second-ball lanes while pivot remains rest defence.
{
...
// Re-run the user defence seeds through the exact visual path and watch for CM/back-line collapse.
for(const [key,seed] of scenarios.slice(0,5)){
 const d=H.createDeveloperScenario({key,seed}),v=A.runDeveloperVisualWindow(d.boundary,{runtimeDir,seed:d.seed,durationSeconds:9}),rows=v.frames||[];let violations=0,frames=0;
...
// Offside developer scenario must actually expose a directed forward option and be able to produce a call.
{
 const key='OFFSIDE_INVOLVEMENT',seed='DEV-RECENT-1787576131788-41',d=H.createDeveloperScenario({key,seed}),o=A.runToChoice(d.boundary,{runtimeDir,seed:d.seed,minPreSeconds:.8,maxSearchSeconds:9}),opts=o?.state?.pending?.options||o?.pending?.options||[],direct=opts.find(x=>['H-ST','H-RW'].includes(x.targetId)&&['THROUGH_PASS','LOFTED_THROUGH_PASS','AVAILABLE_PASS','PROGRESSIVE_PASS'].includes(x.id));
...
const status=checks.every(x=>x.pass)?'PASS':'FAIL';const out={schemaVersion:'FLR_V054_DEFENCE_NORMALIZATION_REGRESSION_1.0',status,checks,watches};fs.writeFileSync(path.resolve(__dirname,'v054_defense_normalization_regression_status.json'),JSON.stringify(out,null,2)+'\n');console.log(JSON.stringify(out,null,2));if(status!=='PASS')process.exitCode=1;
