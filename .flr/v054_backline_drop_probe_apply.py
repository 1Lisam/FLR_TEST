...
if len(sys.argv)<2: raise SystemExit('usage: v054_backline_drop_probe_apply.py <workdir>')
...
def emit(stage):
 ...
 print('V054_BACKLINE_DROP_STAGE='+json.dumps({'stage':stage,'backline':back,'ecology':eco},ensure_ascii=False,separators=(',',':')))
apply('v054_defense_normalization.py');apply('v054_defense_normalization_cm_guard.py');apply('v054_offside_law11_fix.py');emit('BEFORE_DROP_TOGETHER')
apply('v054_backline_drop_together_fix.py');emit('AFTER_DROP_TOGETHER')
print('V054_BACKLINE_DROP_PROBE_APPLIED')
