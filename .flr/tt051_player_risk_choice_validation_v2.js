...
const core=read('runtime/continuous_match_core.js'),ctl=read('runtime/protagonist_match_controller.js'),tactics=read('runtime/tactical_movement.js');
...
add('raw-risk-floor-survives-six-menu',ctl.includes('const riskyRaw=(frame?._frame?.opts||[])')&&ctl.includes('o.block<=1')&&ctl.includes("id:'AVAILABLE_PASS'"));
...
function scenario({lwX=79.0,contested=false}={}){const P=require(path.join(root,'runtime/protagonist_match_controller.js')),s=P.create('QA-PLAYER-RISK',{heroPlayerId:'H-ST',mode:'PLAYER_ALL'}),m=s.m;m.time=1200;m.phase='OPEN_PLAY';m.restart=null;m.completed=false;m.possession='HOME';m.protagonistExplicitActionRequired=true;const ps=m.playersById,hero=ps['H-ST'];for(const p of m.players){if(p.team==='HOME')setPlayer(p,55,34);else setPlayer(p,70,60);}setPlayer(ps['A-LCB'],78.2,61);setPlayer(ps['A-RCB'],78.0,58);setPlayer(hero,72,34);hero.hasBall=true;hero.controlledSince=m.time-1.2;hero.nextThink=m.time;hero.action='HOLD_BALL';hero.tacticalTask='HOLD_BALL';m.ball={...m.ball,mode:'CONTROLLED',x:72.45,y:34,z:0,vx:0,vy:0,vz:0,ownerId:hero.id,intendedReceiverId:null,kind:'CONTROL',lastTouchTeam:'HOME',lastTouchPlayer:hero.id,age:0};m.ballOwner=hero.id;m.lastTouchTeam='HOME';m.lastTouchPlayer=hero.id;const lw=ps['H-LW'];setPlayer(lw,lwX,12);lw.vx=1.8;lw.tx=lwX+2;lw.runUntil=m.time+2;lw.tacticalTask='FAR_SIDE_RUN';lw.action='FAR_SIDE_RUN';const rw=ps['H-RW'];setPlayer(rw,76,46);rw.tacticalTask='MOVE_TO_RECEIVE';rw.action='MOVE_TO_RECEIVE';for(const id of ['H-LCM','H-CM','H-RCM','H-LB','H-RB','H-LCB','H-RCB']){const p=ps[id];if(p){p.x=58;p.y=id.includes('L')?24:54;p.tx=p.x;p.ty=p.y;}}if(contested)setPlayer(ps['A-CM'],74,40);else setPlayer(ps['A-CM'],70,60);const q=P.inspect(s);return{s,m,q};}
try{
 const marginal=scenario({lwX:79.0});const rawMarg=marginal.q?.frame?._frame?.opts?.find(o=>o.p?.id==='H-LW'),uiMarg=marginal.q?.options?.find(o=>o.targetId==='H-LW');
 ...
 const extreme=scenario({lwX:80.7});const rawExtreme=extreme.q?.frame?._frame?.opts?.find(o=>o.p?.id==='H-LW'),uiExtreme=extreme.q?.options?.find(o=>o.targetId==='H-LW');
 ...
 const contested=scenario({lwX:80.7,contested:true});const rawRw=contested.q?.frame?._frame?.opts?.find(o=>o.p?.id==='H-RW'),uiRw=contested.q?.options?.find(o=>o.targetId==='H-RW');
...
const failures=checks.filter(x=>!x.ok),out={schemaVersion:'FLR_TT051_PLAYER_RISK_CHOICE_VALIDATION_1.2',pass:!failures.length,checks,failures};console.log(JSON.stringify(out,null,2));process.exit(failures.length?1:0);
