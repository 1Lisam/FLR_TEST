...
const status=checks.every(x=>x.pass)?'PASS':'FAIL';const out={schemaVersion:'FLR_V053_HF3_MOTION_REGRESSION_1.1',status,checks,cases:cases.map(c=>({seed:c.seed,key:c.key,metrics:c.metrics})),earlyMetrics,offside};fs.writeFileSync(path.resolve(__dirname,'v053_hf3_motion_regression_status.json'),JSON.stringify(out,null,2)+'\n');console.log(JSON.stringify(out,null,2));if(status!=='PASS')process.exitCode=1;
