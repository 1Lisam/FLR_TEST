...
const c=document.querySelector('#qaPitch'),x=c.getContext('2d'),meta=document.querySelector('#meta');let match=null,tick=0,eventSeen=new WeakSet(),config=null;
...
function applyFixture(m,f){
  ...
  // QA-only entrypoint for reviewing the real restart setup, approach and live
  // release when a short natural trace contains no FREE_KICK.  It invokes the
  // public restart bridge; it does not supply a kicker, target, contact, or result.
  if(f.kind==='FREE_KICK_VISUAL'){
    const bridge=E.choiceActionBridge?.(),team=f.team==='HOME'?'HOME':'AWAY',x=Number.isFinite(f.x)?f.x:(team==='HOME'?62:43),y=Number.isFinite(f.y)?f.y:34;
    if(!bridge?.startDeadRestart)throw new Error('FREE_KICK_VISUAL_BRIDGE_UNAVAILABLE');
    // Keep the fixture outside the protagonist's restart-choice pause so this
    // diagnostic can show the ordinary non-protagonist release path end to end.
    m.protagonistControllerId=f.protagonistControllerId||'H-ST';
    bridge.startDeadRestart(m,'FREE_KICK',team,x,y,null,{freeKickType:f.freeKickType||'DIRECT'});
    return;
  }
  if(f.kind==='CORNER_VISUAL'){
    const bridge=E.choiceActionBridge?.(),team=f.team==='HOME'?'HOME':'AWAY',x=Number.isFinite(f.x)?f.x:(team==='HOME'?105:0),y=Number.isFinite(f.y)?f.y:0;
    if(!bridge?.startDeadRestart)throw new Error('CORNER_VISUAL_BRIDGE_UNAVAILABLE');
    m.protagonistControllerId=f.protagonistControllerId||'H-ST';
    bridge.startDeadRestart(m,'CORNER',team,x,y); return;
  }
...
function init(o){config=o;tick=0;eventSeen=new WeakSet();match=E.createMatch(o.seed,o.mode==='D_EXECUTE'?{executionMode:'D_EXECUTE'}:{});for(const p of match.players)if(A?.assign)A.assign(match,p.id,A.baseProfile(60));if(M?.init)M.init(match,{HOME:'BALANCED',AWAY:'BALANCED'});match.offBallPolicy='CURRENT';match.finalThirdDeadlockDiagnostic=!!o.finalThirdDeadlockDiagnostic;applyFixture(match,o.fixture);draw();return {ok:true,players:match.players.map(p=>({id:p.id,team:p.team,role:p.role,slot:p.slot})),time:match.time,executionMode:match.executionMode||o.mode};}
function row(){const b=match.tacticalExecutionBoundary||{},contracts=new Map((b.contracts||[]).map(r=>[r.actorId,r]));const pRows=match.players.map(p=>{const q=contracts.get(p.id)||{},f=p.finalMovementIntent||{};return{id:p.id,team:p.team,role:p.role,slot:p.slot,x:finite(p.x),y:finite(p.y),vx:finite(p.vx),vy:finite(p.vy),body:finite(p.bodyAngle),facing:finite(p.faceTargetAngle),tx:finite(p.tx),ty:finite(p.ty),finalMovementIntent:{type:f.type||null,targetId:f.targetId||null,targetPoint:clone(f.targetPoint)||null,executionMode:f.executionMode||null,contractId:f.contractId||null},d2:{mode:q.executionMode||null,duty:q.duty||null,relationshipTargetId:q.relationshipTargetId||null,targetPoint:clone(q.targetPoint)||null,contractId:q.epochId||null},responsibility:{markTargetId:p.markTargetId||null,targetId:p.responsibilityTargetId||null,type:p.responsibilityType||null,task:p.tacticalTask||null,action:p.action||null},movingReceiveApproach:clone(p.movingReceiveApproach)||null};});const ev=[];for(const e of (match.events||[])){if(eventSeen.has(e))continue;eventSeen.add(e);ev.push(clone(e));}return{type:'tick',tickIndex:tick,simTime:Number(match.time.toFixed(6)),executionMode:b.executionMode||match.executionMode||config.mode,phase:match.phase||null,possession:match.possession||null,restart:clone(match.restart),setPiece:clone(match.setPieceLive),ball:{mode:match.ball.mode||null,x:finite(match.ball.x),y:finite(match.ball.y),vx:finite(match.ball.vx),vy:finite(match.ball.vy),targetX:finite(match.ball.targetX),targetY:finite(match.ball.targetY),ownerId:match.ball.ownerId||null,intendedReceiverId:match.ball.intendedReceiverId||null,currentReceiverId:match.ball.flightReceiverId||null},protagonist:{controllerId:match.protagonistControllerId||null,paused:!!match.paused,choiceId:match.userChoiceControl?.choiceId||match.userChoiceControl?.choice||match.userIncomingIntent?.choiceId||null,targetId:match.userChoiceControl?.targetId||match.userIncomingIntent?.targetId||null},dEpoch:{id:b.epochId||null,mode:b.executionMode||null},players:pRows,events:ev,attackDecision:(()=>{const q=(match.attackingDecisionTrace||[]).at(-1);return q&&Math.abs(Number(q.t)-Number(match.time))<0.051?clone(q):null;})(),challengeTrace:(()=>{const q=(match.finalThirdChallengeTrace||[]).at(-1);return q&&Math.abs(Number(q.t)-Number(match.time))<0.051?clone(q):null;})()};}
function step(){if(!match)throw new Error('QA_NOT_INITIALIZED');E.step(match,.05);tick++;draw();return row();}
function fastForwardTo(targetSeconds){
  if(!match)throw new Error('QA_NOT_INITIALIZED');
  const target=Number(targetSeconds);
  if(!Number.isFinite(target)||target<match.time)throw new Error('QA_FAST_FORWARD_TARGET_INVALID');
  const maxSteps=Math.ceil((target-match.time)/.05)+64;let steps=0;
  while(!match.completed&&match.time+1e-9<target&&steps++<maxSteps){E.step(match,.05);tick++;}
  for(const e of (match.events||[]))eventSeen.add(e);
  draw();
  return{ok:true,time:Number(match.time.toFixed(3)),tickIndex:tick,completed:!!match.completed};
}
function draw(){
  if(!match)return;
...
window.FLR_V57_FRAME_TICK_QA={init,step,fastForwardTo,draw,engineReady:()=>!!E};
})();
