...
function reservedCoverageIds(candidates){
  ...
  // Retain one midfielder for a danger mark and another for the edge. A full
  // five-player wall still has enough full-backs, a midfielder and wingers.
  const midfield=candidates.filter(p=>p.role==='CM').sort((a,b)=>(a.slot==='CM'?-1:b.slot==='CM'?1:0)||a.id.localeCompare(b.id));
  for(const p of midfield.slice(0,2))ids.add(p.id);
  return ids;
}
