(function(){'use strict';
  ...
  const forbidden=/^(expected|oracle|automated|judgment|pass|fail|verdict|expectedJudgment|category)$/i;
  async function mount(){
    const response=await fetch(manifestUrl,{cache:'no-store'}); if(!response.ok)throw new Error('MANIFEST_LOAD_FAILED');
    const manifest=await response.json();
    const cases=(manifest.cases||[]).map(({id,scenario,seed,renderable})=>({id,scenario,seed,renderable:renderable===true}));
    ...
    cases.forEach((c,i)=>{const b=document.createElement('button');b.type='button';b.className='human-calibration__case';b.dataset.caseId=c.id;b.textContent=c.id;b.addEventListener('click',()=>{list.querySelectorAll('.is-selected').forEach(x=>x.classList.remove('is-selected'));b.classList.add('is-selected');select.value=c.scenario;seed.value=c.seed;note.value='';runButton.click();status.textContent=`${c.id} · play/replay the deterministic scene, then record PASS / FAIL / AMBIGUOUS.`});list.appendChild(b);if(i===0)b.click()});
