(() => {
  ...
  function displayState() {
    const raw = live.readState(), pitch2 = raw.donorPitch;
    if (!pitch2) fail("DISPLAY_PITCH_CONTRACT_MISSING");
    return import_hybrid_v48.default.DonorAdapter.toCanonical(raw, { pitch: pitch2 });
  }
  function pitch() {
    ...
    for (const l of [true, false]) {
      const x = l ? 10 : 701;
      ctx.strokeRect(x, 117, 129, 310);
      ctx.strokeRect(l ? 10 : 787, 202, 43, 140);
    }
  }
  function at(p) {
    return { x: 10 + p.x / 105 * 820, y: 10 + p.y / 68 * 524 };
  }
  function donorView(s) {
    pitch();
    for (const p of s.players) {
      const q = at(p);
  ...
  function aView(s) {
    pitch();
    for (const p of s.players) {
      const q = at(p), a = Number.isFinite(p.renderFacingRadians) ? p.renderFacingRadians : Number.isFinite(p.facingRadians) ? p.facingRadians : 0, on = p.id === harness.protagonistId;
      ctx.fillStyle = p.team === 0 ? "#f35f68" : "#5fa9ff";
      ctx.beginPath();
      ctx.arc(q.x, q.y, 11, 0, Math.PI * 2);
      ctx.fill();
      ctx.strokeStyle = on ? "#fff3ae" : "#dce8f588";
  ...
  var label = { READY: "시작 대기", MACRO_RUNNING: "저해상도 경기 진행", INTERACTIVE_PENDING: "A엔진 선택 장면", FULL_TIME: "경기 종료", FAILED_CLOSED: "안전 중단" };
  var clock = (x) => String(Math.floor(x / 60)).padStart(2, "0") + ":" + String(x % 60).padStart(2, "0");
  var inspect = () => harness?.pending?.session ? import_hybrid_v48.default.ASceneAdapter.inspect(harness.pending.session) : null;
  async function choose(c) {
    if (harness.state !== import_hybrid_v48.default.STATES.INTERACTIVE_PENDING) return;
    const exact = { choiceId: c.id, targetId: c.targetId ?? null, playerId: harness.protagonistId };
    $("choices").replaceChildren();
    $("phase").textContent = "A엔진 장면 실행";
    const out = harness.submitAction(exact);
    for (const frame of out.sceneFrames || []) {
      aView(frame);
  ...
  function render() {
    if (!harness) {
      ...
      $("choices").textContent = visibleError ? "오류: " + visibleError : "경기 시작을 눌러 실제 도너 경기를 만드세요.";
      $("diag").textContent = JSON.stringify({ testOnly: true, lifecycle: "READY", error: visibleError }, null, 2);
      return;
    }
    const pending = harness.pending, a = inspect(), isA = harness.state === import_hybrid_v48.default.STATES.INTERACTIVE_PENDING, s = displayState();
    if (isA) aView(a);
    else donorView(s);
    $("phase").textContent = (label[harness.state] || harness.state) + " · " + clock(harness.hostClockStatus().seconds);
    $("choices").replaceChildren(...(isA ? pending.candidates : []).map((c) => {
      const b = document.createElement("button");
      b.textContent = (CHOICE_KO[c.id] || c.id) + (c.targetId != null ? " → 대상 " + c.targetId : "");
      b.onclick = () => choose(c);
      return b;
    }));
    $("diag").textContent = JSON.stringify({ testOnly: true, renderAuthority: isA ? "retained pending.session / ASceneAdapter.inspect(pending.session)" : "donor macro readState", exactChoiceTarget: "pending candidates only", noFuturePreselection: true, protagonistAuthority: true, handbacks: harness.handoffCount, error: visibleError || harness.error || null }, null, 2);
  }
  function tick() {
    if (playing && harness?.state === import_hybrid_v48.default.STATES.MACRO_RUNNING) for (let i = 0; i < speed && harness.state === import_hybrid_v48.default.STATES.MACRO_RUNNING; i++) harness.advanceMacro();
    render();
  }
  $("seed").value = "496001";
  $("reset").onclick = () => {
    ...
    try {
      ...
      visibleError = e?.code || e?.message || String(e);
    }
    render();
  };
  $("play").onclick = () => {
  ...
  $("replay").onclick = async () => {
    if (harness) for (const f of harness.replay.current()) {
      donorView(f);
      await new Promise((r) => setTimeout(r, 18));
    }
  };
  window.__V48_REAL_A_VIEWER_TEST__ = { status: () => ({ state: harness?.state || "READY", error: visibleError || harness?.error || null, pending: harness?.pending?.candidates || [], handbacks: harness?.handoffCount || 0 }), select: (choiceId, targetId) => choose({ id: choiceId, targetId }) };
  render();
