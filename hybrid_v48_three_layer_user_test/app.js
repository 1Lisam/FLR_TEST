(() => {
  ...
  var require_a_scene_adapter = __commonJS({
    "prototype/hybrid_v48/a_scene_adapter.js"(exports, module) {
      ...
      module.exports = Object.freeze({ SUPPORTED, TERMINALS, makeSceneState, canonicalFromA, buildScenePlayerIdMapping, begin, inspect: inspect2, submit, resolve, run, actionFor, terminalFromEvents, inspectInBounds });
    }
  });
  ...
  var require_hybrid_match_controller = __commonJS({
    "prototype/hybrid_v48/hybrid_match_controller.js"(exports, module) {
      ...
      var HybridMatchController = class {
        ...
        submitAction({ choiceId, targetId: targetId2 = null, aim = null, playerId: playerId2 = this.protagonistId }) {
          ...
          const candidate = this.pending.candidates.find((c) => c.id === choiceId && (c.targetId ?? null) === (targetId2 ?? null));
          if (!candidate) return { ok: false, code: "EXACT_CHOICE_OR_TARGET_REJECTED" };
          if (["PASS", "PROGRESSIVE_PASS", "THROUGH_PASS", "SAFE_PASS"].includes(choiceId) && targetId2 == null) return { ok: false, code: "EXACT_TARGET_REQUIRED" };
          this.state = STATES.SCENE_RUNNING;
          const choice = { choiceId, targetId: targetId2, playerId: playerId2, aim: aim || (choiceId === "SHOT" ? { x: 105, y: 34 } : choiceId === "CARRY" ? { x: Math.min(105, this.pending.canonical.players.find((p) => p.id === playerId2).x + 8), y: this.pending.canonical.players.find((p) => p.id === playerId2).y } : null) };
    ...
    "frl_a_validation/engine_b_candidate.js"(exports, module) {
      (function(root, factory) {
        ...
        function allocateLooseBall(s, team, ours, anchors) {
          const targetId2 = s.ball.flight?.type === "pass" ? s.ball.flight.targetId : null;
          ...
          const intended = targetId2 === null ? null : ours.find((p) => p.id === targetId2);
          ...
          for (const item of selected) publishResponsibility(s, team, item.p, "RECEIVE", local(s, team, predicted.x, predicted.y), "LOOSE_BALL_CLAIM", { subjectId: targetId2, region: item.a, releaseReason: "CLAIM_UNTIL_CONTROL_OR_CORRIDOR_EXIT" });
        }
  ...
  // deploy/v48/.v48-real-a-entry.generated.mjs
  ...
  function displayState() {
    return import_hybrid_v48.default.DonorAdapter.toCanonical(live.readState(), { pitch: live.readState().donorPitch });
  }
  function clearMacro() {
    ctx.clearRect(0, 0, 840, 544);
    ctx.fillStyle = "#0b1320";
    ctx.fillRect(0, 0, 840, 544);
    ctx.fillStyle = "#a8b8c8";
    ctx.font = "bold 22px system-ui";
    ctx.fillText("DONOR · background simulation", 28, 72);
    ctx.font = "16px system-ui";
    var s = displayState(), c = harness.hostClockStatus();
    ctx.fillStyle = "#ecf3f8";
    ctx.fillText("경기 " + String(Math.floor(c.seconds / 60)).padStart(2, "0") + ":" + String(c.seconds % 60).padStart(2, "0") + "  ·  스코어 " + s.score[0] + "–" + s.score[1], 28, 112);
    ctx.fillStyle = "#8193a5";
    ctx.font = "13px system-ui";
    ctx.fillText("도너는 화면 뒤에서만 경기 전체를 진행합니다 · 장면 대기 시 pitch 비활성", 28, 145);
  }
  function canvasPoint(p) {
    return { x: 10 + p.x / 105 * 820, y: 10 + p.y / 68 * 524 };
  }
  function pitch() {
    ctx.clearRect(0, 0, 840, 544);
    ctx.fillStyle = "#11623e";
    ctx.fillRect(0, 0, 840, 544);
    ...
    ctx.stroke();
    ctx.beginPath();
    ctx.arc(420, 272, 71.5, 0, Math.PI * 2);
    ctx.stroke();
    ctx.fillStyle = "rgba(255,255,255,.8)";
    ctx.beginPath();
    ctx.arc(420, 272, 3, 0, Math.PI * 2);
    ctx.fill();
    function box(x, d, w) {
      ctx.strokeRect(10 + x / 105 * 820, 10 + (68 - w) / 2 / 68 * 524, d / 105 * 820, w / 68 * 524);
    }
    box(0, 16.5, 40.32);
    box(88.5, 16.5, 40.32);
    box(0, 5.5, 18.32);
    box(99.5, 5.5, 18.32);
    [11, 94].forEach(function(x) {
      ctx.beginPath();
      ctx.arc(10 + x / 105 * 820, 272, 3, 0, Math.PI * 2);
      ctx.fill();
    });
  }
  function targetId(p) {
    var r = p.responsibility || {};
    var ids = [r.pressureTargetId, r.markTargetId, r.watchTargetId, r.subjectId];
    for (var i = 0; i < ids.length; i++) if (Number.isInteger(ids[i])) return ids[i];
    return null;
  }
  function relations(s, by) {
    s.players.forEach(function(p) {
      var id = targetId(p), t = by[id];
      if (!t) return;
      var a = canvasPoint(p), b = canvasPoint(t);
      ctx.save();
      ctx.strokeStyle = p.duty === "PRESS" ? "#ffe36d" : p.duty === "MARK" ? "#ff9b76" : "#9ed7a5";
      ctx.setLineDash(p.duty === "RECOVERY" ? [6, 4] : []);
      ctx.beginPath();
      ctx.moveTo(a.x, a.y);
      ctx.lineTo(b.x, b.y);
      ctx.stroke();
  ...
  function aView(s) {
    ...
    relations(s, by);
    s.players.forEach(function(p) {
      var q = canvasPoint(p), a = Number.isFinite(p.renderFacingRadians) ? p.renderFacingRadians : Number.isFinite(p.facingRadians) ? p.facingRadians : 0, on = p.id === harness.protagonistId;
      ctx.fillStyle = p.team === 0 ? "#f35f68" : "#5fa9ff";
      ctx.beginPath();
      ctx.arc(q.x, q.y, 11, 0, Math.PI * 2);
      ctx.fill();
      ctx.strokeStyle = on ? "#fff3ae" : "#dce8f5";
      ctx.lineWidth = on ? 2.8 : 1.3;
      ctx.beginPath();
      ctx.moveTo(q.x + Math.cos(a) * 4, q.y + Math.sin(a) * 4);
      ctx.lineTo(q.x + Math.cos(a) * (on ? 20 : 14), q.y + Math.sin(a) * (on ? 20 : 14));
      ctx.stroke();
      ctx.fillStyle = "#08111c";
      ctx.font = "bold 10px system-ui";
      ctx.textAlign = "center";
      ctx.fillText("#" + p.id, q.x, q.y + 3);
      ctx.fillStyle = "#eef5ff";
      ctx.font = "8px system-ui";
      ctx.fillText(p.role || "", q.x, q.y + 20);
    });
    var b = canvasPoint(s.ball);
  ...
  var label = { READY: "시작 대기", MACRO_RUNNING: "DONOR · background simulation", INTERACTIVE_PENDING: "ASTRA / V48 Candidate", SCENE_RUNNING: "ASTRA / V48 Candidate · 장면 실행", FULL_TIME: "경기 종료", FAILED_CLOSED: "안전 중단" };
  ...
  async function choose(c) {
    ...
    var exact = { choiceId: c.id, targetId: c.targetId == null ? null : c.targetId, playerId: harness.protagonistId };
    $("choices").replaceChildren();
    try {
      var out = harness.submitAction(exact);
      for (var i = 0; i < (out.displayFrames || []).length; i++) {
        aView(out.displayFrames[i]);
        await new Promise(function(r) {
          setTimeout(r, 18);
        });
      }
      if (!out.ok) visibleError = out.code || "CHOICE_FAILED";
    } catch (e) {
      visibleError = e && e.code || e.message || String(e);
    }
    render();
  }
  function render() {
    if (!harness) {
      $("phase").textContent = visibleError ? "시작 실패 · " + visibleError : "시작 대기";
      $("choices").textContent = visibleError ? "오류: " + visibleError : "경기 시작을 눌러 시작하세요.";
      return;
    }
    var isA = harness.state === import_hybrid_v48.default.STATES.INTERACTIVE_PENDING || harness.state === import_hybrid_v48.default.STATES.SCENE_RUNNING;
    if (isA) {
      var a = inspect();
      if (a) aView(a);
    } else clearMacro();
    $("phase").textContent = (label[harness.state] || harness.state) + " · " + String(Math.floor(harness.hostClockStatus().seconds / 60)).padStart(2, "0") + ":" + String(harness.hostClockStatus().seconds % 60).padStart(2, "0");
    var pending = harness.pending;
    $("choices").replaceChildren(...(harness.state === import_hybrid_v48.default.STATES.INTERACTIVE_PENDING ? pending.candidates || [] : []).map(function(c) {
    ...
    $("diag").textContent = JSON.stringify({ testOnly: true, state: harness.state, badges: { macro: "DONOR · background simulation", scene: "ASTRA / V48 Candidate" }, renderAuthority: isA ? "retained Candidate Match.inspect()" : "compact donor background state", choiceAuthority: "Legacy Choice Kernel", exactChoiceTarget: "choiceId + targetId", displayFrames: "raw inspect from same retained Match", canonicalState: "only handback authority", donorPausedBeforeA: true, donorAdvancesDuringA: false, handback: "same donor object resumes", legacyInitialV1Bundled: false }, null, 2);
  }
  function tick() {
    if (playing && harness && harness.state === import_hybrid_v48.default.STATES.MACRO_RUNNING) for (var i = 0; i < speed && harness.state === import_hybrid_v48.default.STATES.MACRO_RUNNING; i++) harness.advanceMacro();
    render();
  }
  $("seed").value = "496001";
  ...
  window.__V48_TRUE_THREE_LAYER_TEST_ONLY__ = { status: function() {
    return { state: harness ? harness.state : "READY", error: visibleError || harness && harness.error || null, pending: harness && harness.pending ? harness.pending.candidates : [], handbacks: harness ? harness.handoffCount : 0 };
  }, startMatch: function(seed) {
    $("seed").value = seed || $("seed").value;
    $("reset").click();
  }, select: function(choiceId, targetId2) {
    return choose({ id: choiceId, targetId: targetId2 });
  } };
  render();
