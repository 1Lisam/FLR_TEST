...
function reconcile(m,setup){
  ...
  if(cms.length>=2){
    ...
    for(const p of cms){const q=laneBySlot[p.slot]||{x:85,y:34,task:'FREE_KICK_CENTRAL_SCREEN_HOLD'},w=world(defTeam,q.x,q.y);applyTarget(p,setup.targets[p.id],q.task);if(setup.targets[p.id]){setup.targets[p.id].x=w.x;setup.targets[p.id].y=w.y;applyTarget(p,setup.targets[p.id],q.task);}plan.targetsByResponsibility=plan.targetsByResponsibility||{};plan.targetsByResponsibility[p.id]=`${p.slot||'CM'}_DISTINCT_SECOND_BALL_SCREEN`;}
