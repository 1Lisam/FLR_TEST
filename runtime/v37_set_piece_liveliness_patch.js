...
const VERSION='V37-SET-PIECE-LIVELINESS-0.1';
...
function livelyTarget(m,setup,p,t){const r=m.restart,elapsed=Math.max(0,m.time-(setup.createdAt||m.time)),local=worldToLocal(r.team,t.x,t.y),phase=(hash32(`${m.seed}|${r.kind}|${p.id}`)%6283)/1000,freq=.95+((hash32(`${p.id}|FREQ`)%700)/1000),a=Math.sin(elapsed*freq+phase),b=Math.sin(elapsed*(freq*.73)+phase*1.37);let ax=0,ay=0,task=t.task||p.tacticalTask||'SET_PIECE_SETUP';
  if(r.kind==='CORNER'){
    if(p.team===r.team&&['ST','WF','CM'].includes(p.role)){ax=.58;ay=.82;task='CORNER_ATTACK_JOSTLE';}
    else if(p.team!==r.team&&['CB','FB','CM'].includes(p.role)){ax=.38;ay=.62;task='CORNER_MARK_JOSTLE';}
  }else if(r.kind==='FREE_KICK'){
    if(p.team===r.team&&['ST','WF'].includes(p.role)){ax=.72;ay=.52;task='FREE_KICK_RUN_READY';}
    else if(p.team===r.team&&p.role==='CM'){ax=.34;ay=.38;task='FREE_KICK_SECOND_BALL_READY';}
    else if(p.team!==r.team&&['CB','FB','CM'].includes(p.role)){ax=.34;ay=.46;task='FREE_KICK_TRACK_RUNNER';}
  }
  if(ax===0&&ay===0)return null;
  let q=localToWorld(r.team,clamp(local.x+a*ax,1,104),clamp(local.y+b*ay,1,67));if(r.kind==='FREE_KICK'&&p.team===r.team)q=keepWallClear(m,setup,q);return{...q,task};
}
function applyLiveliness(m,setup){const r=m?.restart;if(!r||!setup||!['CORNER','FREE_KICK'].includes(r.kind)||!setup.targets)return setup;const wallIds=new Set(setup.freeKickWall?.wallPlayerIds||[]);for(const [id,t] of Object.entries(setup.targets)){const p=m.playersById?.[id]||m.players.find(q=>q.id===id);if(!p||id===setup.kickerId||p.role==='GK'||wallIds.has(id)||String(t.task||'').includes('WALL')||String(t.task||'').includes('KICKER'))continue;if(dist(p,t)>3.4)continue;const q=livelyTarget(m,setup,p,t);if(!q)continue;p.tx=q.x;p.ty=q.y;p.action=q.task;p.tacticalTask=q.task;p.sprint=false;const marker=nearestMarker(m,p,r.team);p.markTargetId=marker?.id||null;}return setup;}
...
if(baseDebug)R.debugSummary=function(m){const d=baseDebug(m);if(d&&m?.restart&&['CORNER','FREE_KICK'].includes(m.restart.kind))d.livelinessPatch=VERSION;return d;};
