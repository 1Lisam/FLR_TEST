...
function livelyTarget(m,setup,p,t){const r=m.restart;if(r.kind!=='CORNER')return null;const elapsed=Math.max(0,m.time-(setup.createdAt||m.time)),local=worldToLocal(r.team,t.x,t.y),phase=(hash32(`${m.seed}|${r.kind}|${p.id}`)%6283)/1000,freq=.95+((hash32(`${p.id}|FREQ`)%700)/1000),a=Math.sin(elapsed*freq+phase),b=Math.sin(elapsed*(freq*.73)+phase*1.37);let ax=0,ay=0,task=t.task||p.tacticalTask||'SET_PIECE_SETUP';
