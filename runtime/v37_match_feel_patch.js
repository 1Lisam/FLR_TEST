...
const VERSION='V37-MATCH-FEEL-0.1';
...
function prepareLockedDive(m){const shot=currentShotKeeper(m);if(!shot)return;const {b,gk}=shot;if(b.gkRush||b.onTarget===false)return;if(!b.v37DiveIntent){const side=Math.sign(Number(b.shotTargetY)-Number(gk.y))||1;b.v37DiveIntent={version:VERSION,gkId:gk.id,shotTargetY:Number(b.shotTargetY),side,createdAt:m.time};}
  if(b.gkParryReach||Number(b.age||0)<reactionDelay(m,gk))return;const gap=Math.hypot(Number(b.x)-Number(gk.x),Number(b.y)-Number(gk.y));if(gap>5.0)return;
  const intent=b.v37DiveIntent,delta=clamp(intent.shotTargetY-gk.y,-.90,.90),side=Math.sign(delta)||intent.side||1,targetY=clamp(gk.y+delta,.8,67.2);
  b.gkParryReach={startedAt:m.time,startX:gk.x,startY:gk.y,targetX:gk.x,targetY,displacement:Math.abs(targetY-gk.y),approachStartGap:gap,side,v37LockedIntent:true,v37ShotTargetY:intent.shotTargetY};
  gk.v37DiveProfile={startedAt:m.time,duration:.58,startX:gk.x,startY:gk.y,targetX:gk.x,targetY,side,maxSpeed:clamp(5.7+(value(m,gk,'diving')-60)*.012+(value(m,gk,'agility')-60)*.008,5.2,6.6),recoverUntil:m.time+.76};
}
...
E.step=function(m,dt){const beforeCross=m?.ball?.mode==='FLIGHT'&&m.ball.kind==='CROSS'?{team:m.ball.lastTouchTeam}:null;prepareLockedDive(m);applyDiveProfile(m);antiSkateLooseChasers(m);constrainPostCrossStriker(m);const out=baseStep(m,dt);if(beforeCross&&!(m.ball?.mode==='FLIGHT'&&m.ball.kind==='CROSS'))m.v37RecentCross={team:beforeCross.team,startedAt:m.time,until:m.time+2.8};applyDiveProfile(m);constrainPostCrossStriker(m);return out;};
