...
if(live)R.cornerLiveUpdate=function(m,sp){const ok=live(m,sp),plan=sp?.cornerPlan;if(!plan||sp.kind!=='CORNER')return ok;const sign=sp.team==='HOME'?1:-1;
  for(const [id,role] of Object.entries(plan.roles||{})){
    ...
    if(role==='MARKER'){
      ...
      const left=role.endsWith('LEFT'),right=role.endsWith('RIGHT'),w=R.counterOutletTarget?R.counterOutletTarget(q.team,q,left?'LEFT':right?'RIGHT':'CENTER'):world(q.team,role==='COUNTER_OUTLET'?68:70,left?14:right?54:34);q.tx=w.x;q.ty=w.y;q.action=role==='COUNTER_OUTLET'?'CORNER_COUNTER_OUTLET':left?'CORNER_COUNTER_OUTLET_LEFT':'CORNER_COUNTER_OUTLET_RIGHT';q.tacticalTask=q.action;q.sprint=false;
