...
const D_EXECUTION=(typeof globalThis!=='undefined'&&globalThis.FLRPG_OFFBALL_D_EXECUTION_ADAPTER)||((typeof require==='function')?(()=>{try{return require('./offball_d_execution_adapter.js')}catch(_e){return null}})():null);
...
const EXECUTION_MODES=Object.freeze({LEGACY_EXECUTE:'LEGACY_EXECUTE',D_EXECUTE:'D_EXECUTE'});
...
function rng(seed){let a=hash32(seed)||1,drawCount=0;const next=function(){a+=0x6D2B79F5;let t=a;t=Math.imul(t^t>>>15,t|1);t^=t+Math.imul(t^t>>>7,t|61);const value=((t^t>>>14)>>>0)/4294967296;drawCount++;next.onDraw?.({algorithm:'mulberry32',identity:String(seed),state:a>>>0,drawCount,value});return value;};next.observe=()=>({algorithm:'mulberry32',identity:String(seed),state:a>>>0,drawCount});return next;}
function replaceBall(m,next){
  const lease=m?._resolutionLease,target=lease?.canonicalBall;if(!target){m.ball=next;return m.ball;}
  if(m.ball!==target)throw new Error('V2_CANONICAL_BALL_REFERENCE_LOST');const causalHistory=target.causalHistory;
  for(const key of Object.keys(target))delete target[key];Object.assign(target,next);
  if(!Object.hasOwn(target,'lastTouchPlayer'))target.lastTouchPlayer=target.lastTouchPlayerId??null;
  Object.defineProperty(target,'lastTouchPlayerId',{get(){return this.lastTouchPlayer;},set(value){this.lastTouchPlayer=value;},enumerable:true,configurable:true});if(causalHistory&&!Object.hasOwn(target,'causalHistory'))target.causalHistory=causalHistory;m.ball=target;return target;
}
function authorityTraceMutation(m,writer,families,fn,meta={}){const t=m?._continuousSpatialAuthorityV2;if(!t)return fn();const before=t.capture(m,{resolutionMode:'HIGH_RES_LEGACY'}),out=fn();t.observeMutation({writer,before,after:m,families,meta});return out;}
...
function setControlled(m,p,snap=true,receiveMeta=null){return authorityTraceMutation(m,'runtime/continuous_match_core.setControlled',['ball_physical','possession','player_targets','intent_responsibility'],()=>setControlledLegacy(m,p,snap,receiveMeta),{playerId:p?.id||null,snap:!!snap});}
function setBallFlightLegacy(m,{source,target,kind='PASS',speed=16,loft=0.2,targetPoint=null,deliveryMode=null,curve=0,style=null,groundDragK=null}){
...
function setLooseLegacy(m,x,y,vx,vy,lastTeam,lastPlayer){
  ...
  replaceBall(m,{mode:'LOOSE',x,y,z:0,vx,vy,vz:0,ownerId:null,intendedReceiverId:null,kind:'LOOSE',lastTouchTeam:lastTeam,lastTouchPlayer:lastPlayer,age:0,originX:x,originY:y});m.ballOwner=null;
}
function setLoose(m,x,y,vx,vy,lastTeam,lastPlayer){return authorityTraceMutation(m,'runtime/continuous_match_core.setLoose',['ball_physical','possession','intent_responsibility','challenge_duel_contact'],()=>setLooseLegacy(m,x,y,vx,vy,lastTeam,lastPlayer),{lastTeam,lastPlayer});}
...
function assignShape(m){
  if(executionModeForEpoch(m)==='D_EXECUTE')throw new Error('D3_LEGACY_OFFBALL_ASSIGNMENT_FORBIDDEN_IN_D_EXECUTE');
...
// D2: this is still Legacy A/B/C output.  The function deliberately performs the
// historical per-physics-tick label repair before the common commit, rather than
// allowing the old post-motor helper to rewrite an already sealed duty.
function prepareLegacyBallCarrierCrowdingLabels(m){
  ...
  for(const d of m.players.filter(p=>p.team===defTeam&&p.role!=='GK'&&dist(p,owner)<5.35)){
...
function expirePreSealFacing(m){
  for(const p of m.players)if((p.receiveFacingUntil||0)>0&&m.time>p.receiveFacingUntil){p.receiveFacingUntil=0;if(!['TURNING_SHOT_PREP','POST_SHOT_FOLLOW'].includes(p.action))p.faceTargetAngle=null;}
}
function prepareMovementIntentTargets(m,executionMode=executionModeForEpoch(m)){
  // This remains before the seal: it is a protected on-ball action preparation,
  // not an off-ball authority source.
  if(!m._testOnlyD2BoundaryDisabled){expirePreSealFacing(m);if(executionMode==='LEGACY_EXECUTE')prepareLegacyBallCarrierCrowdingLabels(m);}
  for(const p of m.players){
...
function protectedExecutionContractRef(m,p){
  const refs=[];
  if(m.ball?.ownerId===p.id)refs.push({kind:'BALL_OWNER',ownerId:p.id});
  if(m.ball?.intendedReceiverId===p.id||m.ball?.flightReceiverId===p.id)refs.push({kind:'FLIGHT_RECEIVER',intendedReceiverId:m.ball.intendedReceiverId||null,flightReceiverId:m.ball.flightReceiverId||null});
  if(m.restart?.setup?.kickerId===p.id)refs.push({kind:'RESTART_TAKER',restartKind:m.restart.kind||null,restartStage:m.restart.stage||null,kickerId:p.id});
  const restartRole=m.restart?.setup?.targets?.[p.id];if(restartRole&&/WALL|KICKER/.test(String(restartRole.task||'')))refs.push({kind:'RESTART_ROLE',restartKind:m.restart.kind||null,task:restartRole.task||null});
  if(m.protagonistControllerId===p.id||m.userChoiceControl?.playerId===p.id||m.userIncomingIntent?.playerId===p.id){const ownControl=m.userChoiceControl?.playerId===p.id,ownIncoming=m.userIncomingIntent?.playerId===p.id;refs.push({kind:'PROTAGONIST_CONTROLLER',controllerId:m.protagonistControllerId||null,choiceId:ownControl?m.userChoiceControl.choice:ownIncoming?m.userIncomingIntent.choiceId:null,targetId:ownIncoming?m.userIncomingIntent.targetId||null:null,paused:!!m.paused,explicitChoiceAwaiting:!!m.protagonistExplicitActionRequired});}
  if(m.setPieceLive)refs.push({kind:'SET_PIECE_PHASE',kind:m.setPieceLive.kind||null,team:m.setPieceLive.team||null});
  return refs.length?refs:null;
}
function freezeExecutionContract(row){
  if(row.legacyProvenance)Object.freeze(row.legacyProvenance);if(row.dProvenance){if(row.dProvenance.relationship)Object.freeze(row.dProvenance.relationship);Object.freeze(row.dProvenance);}Object.freeze(row.approvedResponsibility);Object.freeze(row.targetPoint);Object.freeze(row.facing);Object.freeze(row.hold);
  if(row.protectedContractRef){for(const ref of row.protectedContractRef)Object.freeze(ref);Object.freeze(row.protectedContractRef);}
  return Object.freeze(row);
}
function commitLegacyTacticalExecutionBoundary(m,source){
  if(m._testOnlyD2BoundaryDisabled)return null;
  const epoch=Number(m._tacticalExecutionEpoch||0)+1,epochId=`${Number(m.time).toFixed(3)}:${epoch}`;
  const rows=m.players.map(p=>{
    const relationshipTargetId=p.responsibilityTargetId||p.markTargetId||null;
    const explicit=!!p.responsibilityType;
    const row={schemaVersion:'D2_LEGACY_TACTICAL_EXECUTION_CONTRACT_1.0',epochId,tickId:Number(m.time.toFixed(3)),teamId:p.team,actorId:p.id,executionMode:'LEGACY_EXECUTE',legacyPolicy:m.offBallPolicy||'CURRENT',legacyProvenance:{source,policy:'A_B_C_CURRENT_FINAL_STATE',responsibilityEpoch:Number.isFinite(p.responsibilityEpoch)?p.responsibilityEpoch:null},approvedResponsibility:explicit?{representation:'LEGACY_RESPONSIBILITY_REFERENCE',type:p.responsibilityType,targetId:relationshipTargetId,reason:p.responsibilityReason||null}:{representation:'LEGACY_SHAPE_COMPATIBILITY',legacyTask:p.tacticalTask||p.action||'HOLD'},relationshipTargetId,targetPoint:{x:Number.isFinite(p.tx)?Number(p.tx):Number(p.x),y:Number.isFinite(p.ty)?Number(p.ty):Number(p.y)},type:p.tacticalTask||p.action||'HOLD',task:p.tacticalTask||null,action:p.action||null,sprint:!!p.sprint,facing:{faceTargetAngle:Number.isFinite(p.faceTargetAngle)?p.faceTargetAngle:null,bodyDirectionInput:Number.isFinite(p.bodyAngle)?p.bodyAngle:null},hold:{lockTargetUntil:Number(p.lockTargetUntil||0),receiveFacingUntil:Number(p.receiveFacingUntil||0),protectedSource:p.userChoiceControl?.playerId===p.id?'USER_CHOICE_CONTROL':p.userIncomingIntent?.playerId===p.id?'USER_INCOMING_INTENT':null},protectedContractRef:protectedExecutionContractRef(m,p),futureOutcomePrecomputed:false};
    return freezeExecutionContract(row);
  });
  const modeRows=m._testOnlyD2ModeRowsOverride||rows;
  if(new Set(modeRows.map(r=>r.executionMode)).size!==1||modeRows.some(r=>r.executionMode!=='LEGACY_EXECUTE'))throw new Error('D2_MODE_EXCLUSIVITY_VIOLATION');
  m._tacticalExecutionEpoch=epoch;
  m.tacticalExecutionBoundary={schemaVersion:'D2_COMMON_EXECUTION_BOUNDARY_1.0',epochId,tickId:Number(m.time.toFixed(3)),executionMode:'LEGACY_EXECUTE',source,committed:true,committedContractCountByActor:Object.fromEntries(rows.map(r=>[r.actorId,1])),contracts:rows,postSealTacticalMutationCount:0,postMotorBodyCorrections:[],futureOutcomePrecomputed:false};
  for(const row of rows)m.playersById[row.actorId].tacticalExecutionContract=row;
  return m.tacticalExecutionBoundary;
}
function commitDExecutionBoundary(m,source){
  if(m._testOnlyD2BoundaryDisabled)throw new Error('D3_D_EXECUTE_REQUIRES_COMMON_BOUNDARY');
  if(!D_EXECUTION||typeof D_EXECUTION.buildExecutionEpoch!=='function')throw new Error('D3_D_EXECUTION_ADAPTER_UNAVAILABLE');
  const epoch=Number(m._tacticalExecutionEpoch||0)+1,epochId=`${Number(m.time).toFixed(3)}:${epoch}`,built=D_EXECUTION.buildExecutionEpoch(m);
  if(!built.snapshot||!built.decision||built.decision.sameImmutableSnapshotForBothTeams!==true||built.rows.length!==m.players.length)throw new Error('D3_INVALID_SHARED_D_EPOCH');
  const rows=built.rows.map(base=>{
    const p=m.playersById[base.actorId],protectedContractRef=protectedExecutionContractRef(m,p);
    const row={schemaVersion:'V57_D3_D_EXECUTE_CONTRACT_1',epochId,tickId:Number(m.time.toFixed(3)),teamId:base.teamId,actorId:base.actorId,executionMode:'D_EXECUTE',duty:base.duty,dProvenance:{source:'D1_CURRENT_STATE_DECISION',snapshotId:built.decision.snapshotId,decisionSchema:built.decision.schemaVersion,relationship:base.provenance,protectedBy:base.protectedBy||null},approvedResponsibility:{representation:base.protectedBy?'D_PROTECTED_EXTERNAL_CONTRACT':'D1_CURRENT_STATE_DECISION',type:base.duty,targetId:base.relationshipTargetId||null,purpose:base.provenance.purpose||null},relationshipTargetId:base.relationshipTargetId||null,targetPoint:{x:base.targetPoint.x,y:base.targetPoint.y},type:base.type,task:base.task,action:base.action,sprint:base.sprint,facing:{faceTargetAngle:base.facing.faceTargetAngle,bodyDirectionInput:base.facing.bodyDirectionInput},hold:{...base.hold,protectedSource:base.protectedBy||null},protectedContractRef,futureOutcomePrecomputed:false};
    if(!p||row.teamId!==p.team||!row.duty||row.futureOutcomePrecomputed!==false)throw new Error(`D3_INCOMPLETE_D_CONTRACT:${base.actorId}`);
    return freezeExecutionContract(row);
  });
  if(new Set(rows.map(r=>r.actorId)).size!==m.players.length||rows.some(r=>r.executionMode!=='D_EXECUTE'||!Number.isFinite(r.targetPoint.x)||!Number.isFinite(r.targetPoint.y)))throw new Error('D3_MODE_EXCLUSIVITY_VIOLATION');
  m._tacticalExecutionEpoch=epoch;
  // Prior D state is owned only here, after a complete epoch has been formed.
  m.dExecutionRuntime={schemaVersion:'V57_D3_D_RUNTIME_STATE_1',priorDecision:built.nextPriorDecision,lastSnapshotId:built.decision.snapshotId,lastEpochId:epochId,futureOutcomePrecomputed:false};
  m.tacticalExecutionBoundary={schemaVersion:'D2_COMMON_EXECUTION_BOUNDARY_1.0',epochId,tickId:Number(m.time.toFixed(3)),executionMode:'D_EXECUTE',source,committed:true,committedContractCountByActor:Object.fromEntries(rows.map(r=>[r.actorId,1])),contracts:rows,postSealTacticalMutationCount:0,postMotorBodyCorrections:[],dSnapshotId:built.decision.snapshotId,dSnapshot:built.snapshot,dDecision:built.decision,dExecutionAudit:{legacyOffBallAssignmentInvocations:0,legacyActorFallbacks:0,dDecisionCalls:1,sameMotor:'movePlayers'},futureOutcomePrecomputed:false};
  for(const row of rows)m.playersById[row.actorId].tacticalExecutionContract=row;
  return m.tacticalExecutionBoundary;
}
function commitTacticalExecutionBoundary(m,source){return executionModeForEpoch(m)==='D_EXECUTE'?commitDExecutionBoundary(m,source):commitLegacyTacticalExecutionBoundary(m,source);}
function recordPostMotorBodySeam(m,name,before){
  const after=m.players.map(p=>({id:p.id,x:p.x,y:p.y,vx:p.vx,vy:p.vy}));
  const changed=after.filter((p,i)=>p.x!==before[i].x||p.y!==before[i].y||p.vx!==before[i].vx||p.vy!==before[i].vy).map(p=>p.id);
  if(changed.length)m.tacticalExecutionBoundary?.postMotorBodyCorrections.push({writer:name,classification:name==='resolveSpacing'?'PURE_PHYSICS_ONLY':'LEGACY_COMPATIBILITY_BODY_SEAM',actorIds:changed});
}
const LEGACY_COMPATIBILITY_BODY_SEAMS=Object.freeze(['stabilizeMarkingBodies','stabilizeOwnerDefenders','stabilizeBallCarrierDefenderCrowding','stabilizeOffBallDefenderCrowding']);
const D2_BASE_POST_MOTOR_BODY_SEAM_ORDER=Object.freeze(['resolveSpacing',...LEGACY_COMPATIBILITY_BODY_SEAMS]);
function bodyCorrectionPolicyForExecutionMode(mode){
  if(mode!=='LEGACY_EXECUTE'&&mode!=='D_EXECUTE')throw new Error('D2_UNKNOWN_BODY_CORRECTION_EXECUTION_MODE');
  // This is deliberately whole-mode policy.  It is not an actor-level choice and
  // does not create a D execution path: D_EXECUTE is inspectable here only so a
  // future cutover cannot accidentally inherit Legacy compatibility bodies.
  return Object.freeze({executionMode:mode,sharedPhysicalCorrections:Object.freeze(['resolveSpacing']),legacyCompatibilityBodySeams:mode==='LEGACY_EXECUTE'?LEGACY_COMPATIBILITY_BODY_SEAMS:Object.freeze([])});
}
function postMotorBodySeamRunner(name,m,dt){
  if(name==='resolveSpacing')return resolveSpacing(m);
  if(name==='stabilizeMarkingBodies')return stabilizeMarkingBodies(m,dt);
  if(name==='stabilizeOwnerDefenders')return stabilizeOwnerDefenders(m,dt);
  if(name==='stabilizeBallCarrierDefenderCrowding')return stabilizeBallCarrierDefenderCrowding(m,dt);
  if(name==='stabilizeOffBallDefenderCrowding')return stabilizeOffBallDefenderCrowding(m,dt);
  throw new Error(`D2_UNKNOWN_POST_MOTOR_BODY_SEAM:${name}`);
}
function runPostMotorBodyCorrections(m,dt,ledger){
  const executionMode=m.tacticalExecutionBoundary?.executionMode||'LEGACY_EXECUTE',policy=bodyCorrectionPolicyForExecutionMode(executionMode);
  // This narrow test-only switch is an in-process D2-base comparator.  It is
  // legal only for Legacy and preserves the former literal order; it is never a
  // D fallback or a product execution policy.
  const useD2BaseControl=!!m._testOnlyD2BaseBodySeamControl;
  if(useD2BaseControl&&executionMode!=='LEGACY_EXECUTE')throw new Error('D2_BASE_BODY_SEAM_CONTROL_REQUIRES_LEGACY_EXECUTE');
  const seamNames=useD2BaseControl?D2_BASE_POST_MOTOR_BODY_SEAM_ORDER:[...policy.sharedPhysicalCorrections,...policy.legacyCompatibilityBodySeams];
  const boundary=m.tacticalExecutionBoundary;
  if(boundary){boundary.bodyCorrectionPolicy={executionMode:policy.executionMode,sharedPhysicalCorrections:[...policy.sharedPhysicalCorrections],legacyCompatibilityBodySeams:[...policy.legacyCompatibilityBodySeams]};boundary.postMotorBodySeamExecutions=[];boundary.postSealTacticalMutationAttempts=[];}
  for(const name of seamNames){
    const before=m.players.map(p=>({id:p.id,x:p.x,y:p.y,vx:p.vx,vy:p.vy})),tacticalBefore=m.players.map(sealedTacticalFields);
    ledger?.start(m,name,{sourceLayer:'V0.6'});authorityTraceMutation(m,`runtime/continuous_match_core.${name}`,['player_actual'],()=>postMotorBodySeamRunner(name,m,dt));ledger?.end(m,name,{sourceLayer:'V0.6'});recordPostMotorBodySeam(m,name,before);
    const tacticalMutationActorIds=m.players.filter((p,i)=>JSON.stringify(sealedTacticalFields(p))!==JSON.stringify(tacticalBefore[i])).map(p=>p.id);
    boundary?.postMotorBodySeamExecutions.push({writer:name,classification:name==='resolveSpacing'?'PURE_PHYSICS_ONLY':'LEGACY_COMPATIBILITY_BODY_SEAM',tacticalMutationActorIds});
    if(tacticalMutationActorIds.length){boundary?.postSealTacticalMutationAttempts.push({writer:name,actorIds:tacticalMutationActorIds});throw new Error(`D2_POST_SEAL_TACTICAL_MUTATION:${name}`);}
  }
}
function sealedTacticalFields(p){return{responsibilityType:p.responsibilityType||null,responsibilityTargetId:p.responsibilityTargetId||null,responsibilityEpoch:Number.isFinite(p.responsibilityEpoch)?p.responsibilityEpoch:null,responsibilityReason:p.responsibilityReason||null,responsibilityMotionMode:p.responsibilityMotionMode||null,responsibilityRewriteReason:p.responsibilityRewriteReason||null,responsibilityContinuityReason:p.responsibilityContinuityReason||null,markTargetId:p.markTargetId||null,tacticalTask:p.tacticalTask||null,action:p.action||null,tx:Number.isFinite(p.tx)?p.tx:null,ty:Number.isFinite(p.ty)?p.ty:null,sprint:!!p.sprint,faceTargetAngle:Number.isFinite(p.faceTargetAngle)?p.faceTargetAngle:null};}
function finalizeMovementDestinations(m,source='OPEN_PLAY_FINAL_ARBITER'){
  ...
  const actors=[];
  for(const p of m.players){
    const contract=m._testOnlyD2BoundaryDisabled?null:p.tacticalExecutionContract;
    if(!m._testOnlyD2BoundaryDisabled&&(!contract||contract.executionMode!==m.tacticalExecutionBoundary?.executionMode||contract.epochId!==m.tacticalExecutionBoundary?.epochId))throw new Error('D2_COMMIT_REQUIRED_BEFORE_SEAL');
    const tx=contract?.targetPoint?.x??(Number.isFinite(p.tx)?Number(p.tx):Number(p.x)),ty=contract?.targetPoint?.y??(Number.isFinite(p.ty)?Number(p.ty):Number(p.y));
    p.finalMovementIntent={schemaVersion:'FINAL_MOVEMENT_INTENT_2.0',stepId,actorId:p.id,contractId:contract?.epochId||null,executionMode:contract?.executionMode||'LEGACY_EXECUTE',type:contract?.type||p.tacticalTask||p.action||'HOLD',task:contract?.task||p.tacticalTask||null,action:contract?.action||p.action||null,targetId:contract?.relationshipTargetId||p.responsibilityTargetId||p.markTargetId||null,targetPoint:{x:tx,y:ty},sprint:contract?.sprint??!!p.sprint,facing:contract?.facing||{faceTargetAngle:Number.isFinite(p.faceTargetAngle)?p.faceTargetAngle:null,bodyDirectionInput:Number.isFinite(p.bodyAngle)?p.bodyAngle:null},hold:contract?.hold||null,protectedContractRef:contract?.protectedContractRef||null,source,writer:'runtime/continuous_match_core.finalizeMovementDestinations',futureOutcomePrecomputed:false};
  ...
  m.movementIntentArbiter={schemaVersion:'SINGLE_FINAL_MOVEMENT_ARBITER_2.0',stepId,at:Number(m.time.toFixed(3)),source,finalWriterCountByActor:Object.fromEntries(actors.map(a=>[a.actorId,1])),postArbiterFinalWriterCount:0,actors,futureOutcomePrecomputed:false};
  if(m.tacticalExecutionBoundary){m.tacticalExecutionBoundary.sealStepId=stepId;m.tacticalExecutionBoundary.sealedContractCountByActor=Object.fromEntries(actors.map(a=>[a.actorId,1]));m.tacticalExecutionBoundary.sealedTacticalFieldsByActor=Object.fromEntries(m.players.map(p=>[p.id,sealedTacticalFields(p)]));}
...
function movePlayers(m,dt){
  ...
  for(const p of m.players){
    if(m._testOnlyD2BoundaryDisabled&&(p.receiveFacingUntil||0)>0&&m.time>p.receiveFacingUntil){p.receiveFacingUntil=0;if(!['TURNING_SHOT_PREP','POST_SHOT_FOLLOW'].includes(p.action))p.faceTargetAngle=null;}
    ...
    const sealedFacing=m._testOnlyD2BoundaryDisabled?p.faceTargetAngle:(final?.facing?.faceTargetAngle??p.faceTargetAngle),sealedSprint=m._testOnlyD2BoundaryDisabled?p.sprint:(final?.sprint??p.sprint),dx=targetX-p.x,dy=targetY-p.y,d=Math.hypot(dx,dy),scanFacing=onBallScanFacingTarget(m,{...p,action:final?.action||p.action,faceTargetAngle:sealedFacing});
    if(d<0.02){
      p.vx=0;p.vy=0;
      const stationaryFacing=Number.isFinite(sealedFacing)?sealedFacing:scanFacing;
    ...
    let vmax=(ROLE_SPEED[p.role]||7)*movementFactor(paceAttr,0.32)*(sealedSprint?1:0.76);
    const pathFacing=Math.atan2(n.y,n.x),explicitFacing=Number.isFinite(sealedFacing)?sealedFacing:scanFacing,defensiveFacing=!!p._defensiveFacingAuthority&&Number.isFinite(explicitFacing),desiredFacing=defensiveFacing?explicitFacing:(d<0.95&&Number.isFinite(explicitFacing)?explicitFacing:pathFacing),beforeFacing=Number.isFinite(p.bodyAngle)?p.bodyAngle:desiredFacing,facingError=Math.abs(angleDiff(beforeFacing,desiredFacing));p.bodyAngle=approachAngle(beforeFacing,desiredFacing,turnRate*dt);
    ...
    if(finalThirdPaceMode){
      ...
      else if(p.team===owner.team&&sealedSprint)vmax=Math.min(vmax,4.85);
      else if(p.team!==owner.team&&sealedSprint&&['FB','CB','CM'].includes(p.role))vmax=vmax;
    }
    let desired=Math.min(vmax,Math.sqrt(Math.max(0,2*a*d)));
    if(finalThirdPaceMode&&(p.hasBall||sealedSprint)){const arrivalScale=clamp(d/1.35,0.12,1);desired=Math.min(desired,Math.sqrt(Math.max(0,2*a*d))*0.88,vmax*arrivalScale);}
  ...
  ledger?.end(m,'continuous_integrator',{sourceLayer:'V0.6',executor:{accelerationLimit:'ROLE_ACCEL',speedLimit:'ROLE_SPEED'}});
  runPostMotorBodyCorrections(m,dt,ledger);
  if(m.tacticalExecutionBoundary?.sealedTacticalFieldsByActor){const changed=m.players.filter(p=>JSON.stringify(sealedTacticalFields(p))!==JSON.stringify(m.tacticalExecutionBoundary.sealedTacticalFieldsByActor[p.id])).map(p=>p.id);m.tacticalExecutionBoundary.postSealTacticalMutationCount=changed.length;m.tacticalExecutionBoundary.postSealTacticalMutationActorIds=changed;}
}
function stabilizeBallCarrierDefenderCrowding(m,dt){
  ...
  // D2 moves this Legacy label repair before the tactical commit.  The test-only
  // legacy comparator retains the historical location for equivalence evidence.
  if(m._testOnlyD2BoundaryDisabled)prepareLegacyBallCarrierCrowdingLabels(m);
...
function markingShapeCollapseCount(m,team,override){
  const ps=outfield(m,team),lp=ps.map(p=>{const q=override&&p.id===override.id?{x:p.x,y:override.y}:p;return{p,q:worldToLocal(team,q.x,q.y)};});
  const xs=lp.map(x=>x.q.x),ys=lp.map(x=>x.q.y),mean=a=>a.length?a.reduce((s,x)=>s+x,0)/a.length:0;
  const line=roles=>lp.filter(x=>roles.includes(x.p.role)).map(x=>x.q.x),df=line(['CB','FB']),mf=line(['CM']),fw=line(['WF','ST']);
  const channels=[0,17,34,51,68].map((x,i)=>lp.filter(z=>z.q.y>=x&&z.q.y<(i===4?69:x+17)).length);
  const neighbors=lp.map(a=>lp.filter(b=>Math.hypot(a.q.x-b.q.x,a.q.y-b.q.y)<=10).length);
  const graph=lp.map(a=>lp.map((b,i)=>Math.hypot(a.q.x-b.q.x,a.q.y-b.q.y)<=14?i:-1).filter(i=>i>=0));
  const seen=new Set(),clusters=[];for(let i=0;i<lp.length;i++){if(seen.has(i))continue;const q=[i];let size=0;seen.add(i);while(q.length){const j=q.pop();size++;for(const k of graph[j])if(!seen.has(k)){seen.add(k);q.push(k);}}clusters.push(size);}
  let signals=0;
  if(Math.max(...ys)-Math.min(...ys)<=22)signals++;
  if(Math.max(...xs)-Math.min(...xs)<=25)signals++;
  if(Math.abs(mean(df)-mean(mf))<=9&&Math.abs(mean(mf)-mean(fw))<=9)signals++;
  if(Math.max(...channels)>=5)signals++;
  if(Math.max(...neighbors)>=5)signals++;
  if(Math.max(...clusters)>=8)signals++;
  if(lp.filter(x=>x.q.x>=25&&x.q.x<=82&&Math.abs(x.q.y-34)<=16).length>=8)signals++;
  return signals;
}
function markingShapeGuardExempt(m,team){
  if(m.restart||m.setPieceLive)return true;
  const ballLocal=worldToLocal(team,m.ball.x,m.ball.y);
  if(ballLocal.x>16.5||ballLocal.y<13.84||ballLocal.y>54.16)return false;
  const emergency=new Set(['SAVE','PARRY','BLOCK','CLEARANCE','CROSS_RECEIVE','HEADER_SHOT','DUEL_CONTACT','DUEL_DEFLECTION']);
  return (m.events||[]).some(e=>m.time-Number(e.t)<=1.5&&emergency.has(e.type));
}
function stabilizeMarkingBodies(m,dt){
  ...
  for(const defender of m.players){
    ...
    if(Math.abs(ny-dl.y)>1e-5&&!markingShapeGuardExempt(m,defender.team)){
      const currentSignals=markingShapeCollapseCount(m,defender.team);
      if(currentSignals<5&&markingShapeCollapseCount(m,defender.team,{id:defender.id,y:ny})>=5){
        let amount=ny-dl.y;
        for(let i=0;i<8;i++){
          const candidate=dl.y+amount*.5;
          if(markingShapeCollapseCount(m,defender.team,{id:defender.id,y:candidate})<5){ny=candidate;amount=candidate-dl.y;}
          else amount*=.5;
        }
        if(markingShapeCollapseCount(m,defender.team,{id:defender.id,y:ny})>=5)ny=dl.y;
      }
    }
...
function syntheticLeadPassCandidates(m,owner,opts,existingThroughTargets=new Set(),lifecycle=null){
  ...
  for(const o of opts||[]){
    const p=o.p;if(!p)continue;const reject=reason=>{if(lifecycle)lifecycle.push({stage:'LEAD_GEOMETRY',candidateId:'THROUGH_PASS',targetId:p.id,included:false,reason});};
    if(existingThroughTargets.has(p.id)){reject('DUPLICATE_EXISTING_THROUGH');continue;}if(!['ST','WF','CM','FB'].includes(p.role)){reject('ROLE_CONTEXT_EXCLUDED');continue;}
    ...
    if(!committed){reject('NO_CAUSAL_RUN_COMMITMENT');continue;}if(!o.lead||o.leadForward<2.5){reject('NO_FORWARD_LEAD_POINT');continue;}if(o.d>44){reject('LEAD_UNREACHABLE_DISTANCE');continue;}if(o.open<0.85){reject('RECEIVER_PRESSURE_FLOOR');continue;}
    const lead={x:o.lead.x,y:o.lead.y},receiverTravel=dist(p,lead);if(receiverTravel<2.2){reject('LEAD_NOT_MATERIALLY_DIFFERENT_FROM_FEET');continue;}if(receiverTravel>17.0){reject('RECEIVER_CANNOT_REACH_LEAD');continue;}
    ...
    if(blockers.length>1){reject('LEAD_LANE_BLOCKED');continue;}if(leadOpen<1.35){reject('LEAD_POINT_OCCUPIED');continue;}
    ...
    const row={id:'THROUGH_PASS',score:Number(score.toFixed(3)),reason:'user_visible_committed_run_lane',meta:{targetId:p.id,targetSlot:p.slot,runLead:true,leadX:Number(lead.x.toFixed(3)),leadY:Number(lead.y.toFixed(3)),leadForward:Number(o.leadForward.toFixed(3)),receiverTravel:Number(receiverTravel.toFixed(3)),receiverPressure:Number(o.open.toFixed(3)),leadOpen:Number(leadOpen.toFixed(3)),leadBlockers:blockers.length,runnerTask:p.tacticalTask||null}};rows.push(row);if(lifecycle)lifecycle.push({stage:'LEAD_GEOMETRY',candidateId:row.id,targetId:p.id,included:true,reason:'CAUSAL_LEAD_REACHABLE'});
  }
...
function recordAttackingDecisionTrace(m,owner,ctx,ranked,selected,reason){
  const shot=ctx?.shot||{},row={t:Number(m.time.toFixed(3)),playerId:owner.id,team:owner.team,role:owner.role,currentState:{distanceToGoal:Number.isFinite(shot.dGoal)?Number(shot.dGoal.toFixed(3)):null,centrality:Number.isFinite(shot.centrality)?Number(shot.centrality.toFixed(3)):null,pressure:Number.isFinite(ctx?.pressure)?Number(ctx.pressure.toFixed(3)):null,shotLaneBlockers:Number(shot.blockers||0),openWindow:!!shot.openWindow,oneVOne:!!shot.oneVOne,clearKeeperChance:!!shot.clearKeeperChance,turningRequired:!!shot.turningRequired,gkRelation:ctx?.gkRelation||null},alternatives:(ranked||[]).filter(c=>c.id!=='SHOT').slice(0,4).map(c=>({id:c.id,score:c.score,targetId:c.meta?.targetId||null,reason:c.reason||null})),selected:{type:selected?.type||null,kind:selected?.kind||null,targetId:selected?.target?.id||null,reason:selected?.reason||reason||null},selectionReason:reason||selected?.reason||null,futureOutcomePrecomputed:false};
  m.attackingDecisionTrace=m.attackingDecisionTrace||[];m.attackingDecisionTrace.push(row);if(m.attackingDecisionTrace.length>40)m.attackingDecisionTrace.shift();return row;
}
...
function midfieldLongShotAction(m,owner,shot,pressure,held){
  ...
  let chance=0.045;
  if(shot.blockers.length===0)chance+=0.025;
  if(shot.dGoal<=25.0)chance+=0.018;
  if(shot.dGoal<=22.0)chance+=0.012;
  if(pressure>=2.8)chance+=0.010;
  if(pressure>=3.8)chance+=0.008;
  if(['EDGE_SHOT','SECOND_BALL','BOX_EDGE_SUPPORT','CUTBACK_EDGE','SECOND_LINE_SUPPORT','SECOND_WAVE_8','ADVANCE_SUPPORT','LATE_BOX'].includes(owner.tacticalTask))chance+=0.018;
  chance=clamp(chance,0.045,0.16);
...
function chooseOwnerAction(m,owner){
  ...
  if(decisiveReceive){const open=shot.blockers.length===0,baseP=shot.oneVOne?.38:open?(shot.inBox?.030:.024):.016,roleP=owner.role==='ST'?0.020:owner.role==='WF'?0.006:-0.006,pressureP=pressure>=3.2?.012:pressure>=2.0?.006:pressure<1.2?-.012:0,p=clamp(baseP+roleP+pressureP,.012,.58),roll=(hash32(`${m.seed}|DECISIVE_RECEIVE_SHOT|${Math.floor((owner.controlledSince||m.time)*10)}|${owner.id}`)%10000)/10000;if(roll<p){m.stats.decisiveReceiveShots=(m.stats.decisiveReceiveShots||0)+1;return{type:'SHOT',reason:'DECISIVE_RECEIVE_FINISH'};}}
  ...
  // A decisive shot is chosen from current geometry, not from a timer or a target-score rule.
  // Strict 1v1s always outrank recycling; the broader keeper-facing case must still beat the
  // best real pass/cut-back alternative by a visible utility margin.
  const shotCandidate=ranked.find(c=>c.id==='SHOT'),bestAlternative=ranked.find(c=>c.id!=='SHOT'&&!['HOLD','TURN_BACK'].includes(c.id));
  const decisiveShot=!!shotCandidate&&(shot.oneVOne||(shot.clearKeeperChance&&shotCandidate.score>=(bestAlternative?.score??-99)+0.35));
  if(decisiveShot){const selected=candidateToAction(m,owner,shotCandidate,{...pre,deep,early,takeOn,ctx});if(selected){selected.reason=shot.oneVOne?'DECISIVE_ONE_V_ONE':'CLEAR_KEEPER_CHANCE_SUPERIOR';recordCandidateDecision(m,owner,shotCandidate,selected,true);recordAttackingDecisionTrace(m,owner,ctx,ranked,selected,'CURRENT_SHOT_SUPERIOR');return selected;}}
  ...
  if(CANDIDATES&&local.x>=70){
    ...
    const topUtility=ranked[0]?.score??-99,competitive=ranked.filter(c=>c.score>=topUtility-.70).slice(0,2);
    for(const c of competitive){
      if(c.id==='HOLD'||c.id==='TURN_BACK')continue;
      if(c.id==='SHOT'&&c!==ranked[0])continue;
      ...
      if(c.id==='SHOT'&&owner.candidateShotDecline){
        ...
        const sameControl=Math.abs(Number(sd.controlledSince)-Number(owner.controlledSince))<.001;
        if(sameControl&&!improved)continue;
        owner.candidateShotDecline=null;
      }
      ...
      if(roll>commit){
        ...
        if(c.id==='SHOT'&&!shot.oneVOne&&shot.dGoal>9.8){const central=Math.abs(local.y-34),critical=shot.inBox&&shot.blockers.length===0&&shot.dGoal<=18.5&&central<=11.0;owner.candidateShotDecline={until:m.time+(critical?1.65:clamp(2.6+(shot.dGoal>17?0.65:0)+(central>12?0.55:0),2.6,4.0)),controlledSince:owner.controlledSince,dGoal:shot.dGoal,centrality:central};}
    ...
    if(genuineOpenChance){const selected={type:'CARRY',reason:'PRESERVE_OPEN_ATTACKING_CHANCE'};recordCandidateDecision(m,owner,ranked.find(c=>c.id==='CARRY')||ranked[0]||null,selected,true);recordAttackingDecisionTrace(m,owner,ctx,ranked,selected,'SHOT_NOT_YET_SUPERIOR');return selected;}
  }
...
function executeShot(m,owner,reason='GENERAL',exec={}){
  ...
  let onTargetP=clamp(0.225+assess.score*0.019-assess.dGoal*0.0040+(finishing-60)*0.0022,0.18,0.54);
  ...
  const onTarget=m.r()<clamp(onTargetP,0.16,keeperBeaten?0.93:(highQualityKeeperChance?0.91:0.56));if(onTarget)m.stats.shotsOnTarget=(m.stats.shotsOnTarget||0)+1;let aimY;
  if(onTarget){
  ...
  if((owner.lockTargetUntil||0)>m.time&&['DRIBBLE_EVADE','CARRY_FORWARD','COMMITTED_BOX_CARRY','TAKE_ON','TURN_BACK','DUEL_ESCAPE','FIRST_TOUCH_FLOW','TURNING_SHOT_PREP'].includes(owner.action)){
    if(['DRIBBLE_EVADE','CARRY_FORWARD','COMMITTED_BOX_CARRY','TAKE_ON','FIRST_TOUCH_FLOW'].includes(owner.action)&&m.time-(owner.lastCarryInterruptCheck||-99)>=(owner.action==='TAKE_ON'?0.14:0.22)){
      ...
      if(owner.action!=='TAKE_ON'||!owner.takeOnState){
        ...
        // Carry execution may release into a new decision, but must not run a fresh shot lottery
        // every 0.22 s. Only an actual 1v1, close-range lane, or completed take-on can finish
        // directly; ordinary open windows return to chooseOwnerAction for one ranked decision.
        if(si.inBox&&((si.oneVOne)||(takeOnFinish&&m.r()<(si.dGoal<=11.5?0.98:0.82))||(si.dGoal<=9.5&&si.blockers.length===0))){owner.lockTargetUntil=0;owner.boxCarryCommittedUntil=0;if(takeOnBreak)m.stats.takeOnBreakawayShots=(m.stats.takeOnBreakawayShots||0)+1;applyResolvedOwnerAction(m,owner,{type:'SHOT',reason:takeOnBreak?(si.oneVOne?'TAKE_ON_ONE_V_ONE':'TAKE_ON_SHOT_WINDOW'):(si.oneVOne?'ONE_V_ONE':'CARRY_SHOT_WINDOW')});return;}
      }
...
function activePresserNearOwner(m,owner,radius=2.2){
  ...
  const cands=outfield(m,other(owner.team)).map(p=>({p,d:dist(p,owner)}))
    // Proximity alone is physical contact, not a duel.  The episode may begin only
    // after the current final defensive task has become challenge-eligible.
    .filter(o=>o.d<=radius&&['ENGAGE','CLOSE_DOWN','CHASE_LOOSE'].includes(o.p.tacticalTask||o.p.action))
    .sort((a,b)=>a.d-b.d);
  return cands[0]||null;
}
function challengeFacingAlignment(defender,owner){
  if(!Number.isFinite(defender?.bodyAngle)||!owner)return 1;
  return Math.cos(angleDiff(defender.bodyAngle,Math.atan2(owner.y-defender.y,owner.x-defender.x)));
}
...
function forceResolveDuel(m,owner,defender){
  ...
  // automatic shot/pass/carry. End only the defender's contact episode so the protagonist
  // controller can expose the next explicit choice without bodies remaining pinned together.
  if(m.protagonistExplicitActionRequired===true&&m.protagonistControllerId===owner.id){
    // A bounded defender handback preserves the protagonist's untouched possession
    // rather than resolving it with an AI shot/pass/carry.  The pair cooldown moves the
    // defender back to a contain distance on the next current-state assignment.
    setPairCooldown(m,defender,owner,1.25);defender.pressRecoverUntil=Math.max(defender.pressRecoverUntil||0,m.time+.65);m.stats.duelExplicitProtagonistHandbacks=(m.stats.duelExplicitProtagonistHandbacks||0)+1;return;
  }
...
function resolveNpcGkShot(m,gk,prev){
  ...
  // A live rush owns its own contact route. Other keeper-facing chances still use this
  // ability/difficulty contest when no physical rush contact was created; bypassing both
  // routes made open 14-18 m shots cross the line without any goalkeeper contest.
  if(gk.action==='GK_RUSH'||gk.tacticalTask==='GK_RUSH')return null;
  ...
  const saveChance=clamp(.90+contest*.007-difficulty*.006,.52,.97),saveRoll=(hash32(`${key}|CONTEST`)%1000000)/1000000;
...
function updateLooseChasers(m){
  if(executionModeForEpoch(m)==='D_EXECUTE')throw new Error('D3_LEGACY_LOOSE_BALL_ASSIGNMENT_FORBIDDEN_IN_D_EXECUTE');
...
function step(m,dt=m.dt){
  if(m.completed)return m;const executionMode=executionModeForEpoch(m);dt=clamp(Number(dt)||m.dt,0.02,0.15);m._continuousSpatialAuthorityV2?.beginEpoch('CORE_STEP',{id:`CORE_STEP:${Number(m.time+dt).toFixed(3)}`,dt});
  ...
  authorityTraceMutation(m,'runtime/continuous_match_core.step.clock',['clock_phase_score'],()=>{m.time+=dt;});
  if(m.time>=5400){
  ...
  if(m.ball.mode==='CONTROLLED'){const owner=playerById(m,m.ball.ownerId);if(owner){
    ...
    if(!controllerPreempt)authorityTraceMutation(m,'runtime/continuous_match_core.ownerThink',['ball_physical','possession','player_targets','intent_responsibility'],()=>ownerThink(m,owner));
  }}
  ...
  if(TELEMETRY&&m.telemetry&&typeof TELEMETRY.tick==='function')TELEMETRY.tick(m);m._continuousSpatialAuthorityV2?.sample(m,{resolutionMode:'HIGH_RES_LEGACY'});m._continuousSpatialAuthorityV2?.endEpoch();
  return m;
}
...
function incomingBallChoiceState(m,owner){
  ...
  if(closing<=0.45)return null;const receiveRadius=(CONTROL_RADIUS[owner.role]||1.05)+(kind==='CROSS'?.20:.72),eta=Math.max(.055,(d-receiveRadius)/closing);if(eta>1.45)return null;
...
function inspectChoiceState(m,playerId){
  ...
  if(m.ball.mode==='CONTROLLED'&&m.ball.ownerId===owner.id){
    const lifecycle=[],local=worldToLocal(owner.team,owner.x,owner.y),pressure=ballCarrierPressureDistance(m,owner),space=forwardSpace(m,owner,13),shot=shotAssessment(m,owner),opts=passOptions(m,owner,'PLAYER'),held=Math.max(0,m.time-(owner.controlledSince||m.time)),deep=finalThirdDelivery(m,owner),early=earlyCrossDelivery(m,owner),takeOn=takeOnOpportunity(m,owner,shot,held),ctx=candidateContext(m,owner,shot,opts,pressure,space,held,deep,early,takeOn),ranked=candidateRank(m,owner,ctx);
    for(const p of teamPlayers(m,owner.team)){if(p.id===owner.id)continue;const o=opts.find(x=>x.p.id===p.id);let reason='GENERATED_LEGAL_GEOMETRY';if(!o){const d=dist(owner,p),line=offsideLine(m,owner.team),offside=owner.team===HOME?(p.x>Math.max(52.5,m.ball.x+.25,line+.25)):(p.x<Math.min(52.5,m.ball.x-.25,line-.25));reason=p.role==='GK'&&local.x>45?'GK_CONTEXT_EXCLUDED':d<3?'TOO_CLOSE':d>52?'UNREACHABLE_DISTANCE':offside?'OFFSIDE_EXCLUDED':'PASS_GEOMETRY_EXCLUDED';}else if(o.block>1)reason='LANE_BLOCKED';else if(o.open<.35)reason='RECEIVER_PRESSURE_FLOOR';lifecycle.push({stage:'GENERATION_LEGALITY',candidateId:'PASS_TARGET',targetId:p.id,included:!!o&&o.block<=1&&o.open>=.35,reason,geometry:o?{distance:Number(o.d.toFixed(3)),forward:Number(o.forward.toFixed(3)),laneBlockers:o.block,receiverSpace:Number(o.open.toFixed(3)),offsideRisk:!!o.offsideRisk}:null});}
...
function choiceActionBridge(){
  ...
  return{
    ...
    executeShot,executeCarry,executeTakeOn,takeOnOpportunity,executePass,setControlled,setLoose,startDeadRestart,startGoalCelebration,handleOut,event,shotAssessment,chooseOwnerAction
  };
}
...
return{createMatch,step,snapshot,runToEnd,choiceActionBridge,choiceStateBridge,setExecutionMode,executionModeForEpoch,bodyCorrectionPolicyForExecutionMode,FIELD,HOME,AWAY,DEFAULT_DT};
});
