...
function readiness(m){
  ...
  const principalRunnerIds=setup.kind==='FREE_KICK'?setup.freeKickPlan?.principalRunnerIds||[]:[],principalRunnersReady=principalRunnerIds.every(id=>{const p=playerById(m,id),t=setup.targets[id];return!!(p&&t&&dist(p,t)<=2.65);}),ratio=total?readyN/total:1,forced=m.time>=setup.maxReadyAt,wrongEndRequiredTarget=hasWrongEndRequiredTarget(m,setup),requiredRatio=setup.kind==='CORNER'?0.90:setup.kind==='PENALTY'?0.90:0.72,ready=kickerReady&&facingReady&&!wrongEndRequiredTarget&&principalRunnersReady&&((m.time>=setup.minReadyAt&&ratio>=requiredRatio)||forced);setup.readyRatio=ratio;return{ready,ratio,kickerReady,facingReady,forced,wrongEndRequiredTarget,principalRunnerIds,principalRunnersReady,requiredRatio,stage:r.stage||'SETUP',kickerPosition:kicker?{x:Number(kicker.x.toFixed(3)),y:Number(kicker.y.toFixed(3))}:null,kickerTarget:kickerTarget?{x:Number(kickerTarget.x.toFixed(3)),y:Number(kickerTarget.y.toFixed(3))}:null,kickerDistance:kicker?(approachToBall?dist(kicker,{x:r.x,y:r.y}):kickerTarget?dist(kicker,kickerTarget):null):null,elapsed:m.time-setup.createdAt,minReadyAt:setup.minReadyAt,maxReadyAt:setup.maxReadyAt};
}
