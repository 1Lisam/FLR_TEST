...
function capInvertedFullbackForWideThreat(m,p,target){
  ...
  const cover=outfield(m,p.team).some(q=>{
    if(q.id===p.id||!['CB','CM','FB'].includes(q.role))return false;
    const ql=worldToLocal(p.team,q.x,q.y);
...
function roleEligibleForThreat(m,d,threat){
  ...
  if(threat.kind==='CENTRAL'){
    ...
    if(d.role==='FB'){
      ...
      if(wide&&wl&&['WF','FB'].includes(wide.role)&&wl.x<=49&&Math.abs(wl.y-34)>=15)return false;
    }
  ...
  if(threat.kind==='WIDE')return ['FB','CM','CB'].includes(d.role);
  return ['CB','FB','CM'].includes(d.role);
}
...
return{assign,assignLooseBallArbitration,describe,PROFILES,FORMATION,phaseFromProgress,reconcileDefensiveResponsibilities,currentDefensiveThreats,executeDefensiveResponsibilityMotion,capInvertedFullbackForWideThreat};
});
