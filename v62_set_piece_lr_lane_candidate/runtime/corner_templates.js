...
function takePair(a,u,rs,restartTeam,targets){const result=[];for(const i of targets.map((_,i)=>i).sort((i,j)=>Math.abs(targets[j].y-34)-Math.abs(targets[i].y-34)||i-j))result[i]=takeLane(a,u,rs,restartTeam,targets[i]);return result;}
