...
function pushHistory(s){
  const f=s.fastReplayHistory?fastReplayFrame(s.m):leanFrame(s.m),last=s.history[s.history.length-1];
  ...
  s.m._continuousSpatialAuthorityV2?.recordLineage({kind:'HISTORY_APPEND',writer:'runtime/protagonist_match_controller.pushHistory',at:f.time,historyLength:s.history.length,sceneId:s.currentScene?.sceneId||null,pendingChoiceId:s.pending?.id||null});
}
...
function maybeCheckpoint(s){
  updateEpisodeState(s);const def=modeDef(s);if(s.pending||s.resultTracker||def.presentation==='SKIP'||s.m.completed)return null;const q=inspect(s);if(s.m.restart&&(!q||q.frame?.kind!=='RESTART'))return null;
...
function resultNarrative(s,tr,terminal=null){
  ...
  if(isShotChoice(choice)){
    ...
    if(['PARRY','CHIP_PARRY'].includes(ev?.type))return{code:'PARRIED',headline:`${style} → 골키퍼가 쳐냄`,detail:`${ev.text||'골키퍼가 손끝으로 공을 쳐냈습니다.'} 이후 세컨드볼 상황까지 이어졌습니다.`,terminalEvent:deep(ev)};
...
function updateResultTracker(s){
  ...
  if(tt==='GOAL'){if(s.m.phase==='GOAL_CELEBRATION')tr.goalCelebrationObserved=true;if(tr.goalCelebrationObserved&&s.m.phase!=='GOAL_CELEBRATION'&&(!s.m.restart||s.m.restart.kind!=='KICKOFF'))tr.kickoffContinuationObserved=true;ready=!!tr.goalCelebrationObserved&&!!tr.kickoffContinuationObserved;}
    ...
    if(heroOwn&&now>=tr.startedAt+0.75)ready=true;
      ...
      const shotOutcome=downstreamShot&&tr.newEvents.find(e=>e.t>=downstreamShot.t&&['GOAL','SAVE','PARRY','CHIP_SAVE','CHIP_PARRY','BLOCK','CORNER','GOAL_KICK'].includes(e.type));
      if(shotOutcome){
...
function applyChoice(s,choiceId,targetId=null,inputMeta={}){
  ...
  if(s.pending.kind==='RESTART')res=C().applyRestartChoice(s.m,s.heroPlayerId,opt.id,opt.targetId||null,inputSource);
  else if(s.pending.kind==='ON_BALL'&&['SHORT_DISTRIBUTION','LONG_DISTRIBUTION'].includes(opt.id))res=R38.apply(s.m,{playerId:s.heroPlayerId,choice:opt.id,targetId:opt.targetId||null});
  ...
  if(res.ok){
    ...
    if(s.currentScene)s.currentScene.choicePendingSnapshot=deep(s.pending);s.pending=null;beginResultTracker(s,opt,res,before);updateResultTracker(s);s.m._continuousSpatialAuthorityV2?.recordLineage({kind:'CHOICE_APPLY',writer:'runtime/protagonist_match_controller.applyChoice',at:s.m.time,choiceId:opt.id,targetId:opt.targetId||null,inputSource,futureOutcomePrecomputed:false});
  }
  return res;
}
