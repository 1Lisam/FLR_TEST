...
function currentStateSummary(state,attachJson){const match=state?.match||{},ball=state?.ball||{};return{sv:'FLR_CURRENT_STATE_SUMMARY_0.1',captureStatus:state?(attachJson?'BOUNDED_CURRENT_STATE_JSON_REQUESTED':'METADATA_ONLY_USER'):'CURRENT_STATE_UNAVAILABLE',fullDebugAvailable:false,captureKind:state?.captureKind||null,matchSecond:match.second??null,score:match.score||null,possession:match.possession||null,phase:match.phase||null,heroPlayerId:match.heroPlayerId||null,heroRole:match.heroRole||null,activeBoundary:state?.activeBoundary||null,ball:{mode:ball.mode||null,x:ball.x??null,y:ball.y??null,ownerId:ball.ownerId||null,team:ball.team||null,lane:ball.lane||null,progress:ball.progress??null},players:(state?.players||[]).map(p=>[p.id||null,p.team||null,p.role||null,p.slot||null,p.x??null,p.y??null,p.vx??null,p.vy??null,p.action||null,p.tacticalTask||null,p.markTargetId||null]),futureOutcomePrecomputed:state?.futureOutcomePrecomputed===true};}
...
function install(){
  ...
  function prepareUiOnly(){uiOnly=true;forcedMode=false;hideFallback();desc.value='';if(attach){attach.checked=true;attach.disabled=false}modal.hidden=false;setTimeout(()=>desc.focus(),0)}
