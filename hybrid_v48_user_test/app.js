(() => {
  ...
  var require_a_scene_adapter = __commonJS({
    "prototype/hybrid_v48/a_scene_adapter.js"(exports, module) {
      ...
      function canonicalFromA(inspect, options = {}) {
        const mapping = options.scenePlayerIdMapping;
        if (!mapping) throw new Error("A_PLAYER_MAPPING_REQUIRED_FOR_RETURN");
        if (!Array.isArray(inspect.players) || inspect.players.length !== 22 || new Set(inspect.players.map((p) => p.id)).size !== 22) throw new Error("A_RETURN_REQUIRES_22_UNIQUE_PLAYERS");
        for (const p of inspect.players) mapping.canonicalForA(p.id);
        if (inspect.ball.owner != null) mapping.canonicalForA(inspect.ball.owner);
        return Canonical.create({ tick: inspect.tick, clock: inspect.clock, phase: inspect.phase, score: inspect.score, possessionTeam: inspect.possession, players: inspect.players.map((p) => ({ id: mapping.canonicalForA(p.id), team: p.team, role: p.role, x: p.x, y: p.y, held: Math.max(0, inspect.time - p.controlSince) })), ball: { x: inspect.ball.x, y: inspect.ball.y, z: inspect.ball.z, ownerId: inspect.ball.owner == null ? null : mapping.canonicalForA(inspect.ball.owner), mode: inspect.ball.mode }, metadata: { aVersion: inspect.version } });
      }
      function inspectInBounds(inspect) {
        const b = inspect && inspect.ball;
        if (!b || !Number.isFinite(b.x) || !Number.isFinite(b.y) || b.x < 0 || b.x > 105 || b.y < 0 || b.y > 68) return false;
        return Array.isArray(inspect.players) && inspect.players.every((p) => Number.isFinite(p.x) && Number.isFinite(p.y) && p.x >= 0 && p.x <= 105 && p.y >= 0 && p.y <= 68);
      }
      function terminalBoundaryReady(inspect, terminal) {
        if (!terminal || !inspectInBounds(inspect)) return false;
        if (["save", "claim", "block"].includes(terminal.type)) return inspect.ball.owner !== null && !inspect.ball.flight;
        return inspect.phase !== "play" || inspect.ball.owner !== null && !inspect.ball.flight;
      }
      function run(canonical, choice, options = {}) {
        Canonical.validate(canonical);
        const protagonistId = choice.playerId;
        if (canonical.ball.ownerId !== protagonistId) return { ok: false, code: "PROTAGONIST_NOT_CURRENT_OWNER" };
        const action = actionFor(choice);
        if (action.targetId != null && (!Number.isInteger(action.targetId) || action.targetId === protagonistId)) return { ok: false, code: "EXACT_TARGET_REJECTED" };
        const rawTemplate = sceneTemplate(options), mapping = buildScenePlayerIdMapping(canonical, rawTemplate, options), raw = makeSceneState(canonical, { ...options, template: rawTemplate, scenePlayerIdMapping: mapping }), Engine = options.engine, match = Engine.Match.restore(raw), aPlayerId = mapping.aForCanonical(protagonistId), aTargetId = action.targetId == null ? null : mapping.aForCanonical(action.targetId), submittedAction = action.targetId == null ? action : { ...action, targetId: aTargetId }, evidence = { choiceId: choice.choiceId || choice.id, playerId: protagonistId, targetId: action.targetId == null ? null : action.targetId, aPlayerId, aTargetId };
        if (!match.submitAction({ playerId: aPlayerId, ...submittedAction })) return { ok: false, code: "A_REJECTED_ACTION", evidence };
        match.resume();
        const startEvents = match.inspect().eventCount, max = options.maxSteps || 900, observedFrames = [];
        let pendingTerminal = null;
        for (let step = 0; step < max; step++) {
          if (!match.advance()) break;
          const inspect = match.inspect(), events = match.eventsSince(startEvents), observed = terminalFromEvents([...events].reverse()), current = inspectInBounds(inspect) ? canonicalFromA(inspect, { scenePlayerIdMapping: mapping }) : null;
          if (current) {
          ...
          if (pendingTerminal && terminalBoundaryReady(inspect, pendingTerminal)) return { ok: true, terminal: pendingTerminal, canonicalState: current, steps: step + 1, evidence, observedFrames };
          if (choice.choiceId !== "SHOT" && choice.id !== "SHOT" && inspect.phase === "play" && inspect.ball.owner !== null && !inspect.ball.flight && inspectInBounds(inspect) && step >= (options.minimumSteps || 2)) return { ok: true, terminal: { type: "controlledPossession", ownerId: mapping.canonicalForA(inspect.ball.owner) }, canonicalState: current, steps: step + 1, evidence, observedFrames };
        }
        return { ok: false, code: "SCENE_UNRESOLVED_FAIL_CLOSED", evidence };
      }
      module.exports = Object.freeze({ SUPPORTED, TERMINALS, makeSceneState, canonicalFromA, buildScenePlayerIdMapping, run, actionFor, terminalFromEvents });
    }
  });
  ...
  var require_hybrid_match_controller = __commonJS({
    "prototype/hybrid_v48/hybrid_match_controller.js"(exports, module) {
      ...
      var STATES = Object.freeze({ MACRO_RUNNING: "MACRO_RUNNING", INTERACTIVE_PENDING: "INTERACTIVE_PENDING", SCENE_RUNNING: "SCENE_RUNNING", SCENE_COMMIT: "SCENE_COMMIT", REPLAY_AVAILABLE: "REPLAY_AVAILABLE", FAILED_CLOSED: "FAILED_CLOSED" });
      var HybridMatchController = class {
        ...
        advanceMacro() {
          ...
          this.donor.advance();
          const canonical = this.observe(), trigger = presentTrigger(canonical, this.protagonistId);
          if (trigger) {
            this.donor.pause();
            const candidates = Choices.generate(canonical, this.protagonistId);
            this.pending = { canonical, trigger, candidates };
            this.state = STATES.INTERACTIVE_PENDING;
            return { ok: true, interactive: true, candidates };
          }
        ...
        submitAction({ choiceId, targetId = null, aim = null, playerId: playerId2 = this.protagonistId }) {
          ...
          const scene = AScene.run(this.pending.canonical, { choiceId, targetId, playerId: playerId2, aim: aim || (choiceId === "SHOT" ? { x: 105, y: 34 } : choiceId === "CARRY" ? { x: Math.min(105, this.pending.canonical.players.find((p) => p.id === playerId2).x + 8), y: this.pending.canonical.players.find((p) => p.id === playerId2).y } : null) }, { engine: this.aEngine, ...this.aOptions });
          ...
          this.donor.resume();
          this.state = STATES.MACRO_RUNNING;
          return { ok: true, terminal: scene.terminal, state: this.state, replayAvailable: this.replay.captures.length > 0, sceneFrames: scene.observedFrames || [] };
        }
        fail(code, detail = {}) {
          this.error = code;
      ...
      module.exports = Object.freeze({ HybridMatchController, STATES });
    }
  });
  ...
  // deploy/v48/.v48-hybrid-browser-entry.generated.mjs
  ...
  var FLR = "86f8ac5349135325240ebf7f5d761b00e310a337";
  ...
  function tick() {
    if (playing && harness.state === import_hybrid_v48.default.STATES.MACRO_RUNNING) {
      for (let i = 0; i < speed; i++) harness.advanceMacro();
    }
    render();
  }
  $("seed").value = "496001";
