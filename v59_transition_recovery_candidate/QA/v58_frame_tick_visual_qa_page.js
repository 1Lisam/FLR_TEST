...
function fastForwardTo(targetSeconds){
  ...
  return{ok:true,time:Number(match.time.toFixed(3)),tickIndex:tick,completed:!!match.completed,
    digest:{phase:match.phase||null,possession:match.possession||null,score:{...match.score},
      ball:{mode:match.ball.mode||null,ownerId:match.ball.ownerId||null,x:Number(match.ball.x.toFixed(6)),y:Number(match.ball.y.toFixed(6))},
      players:match.players.map(p=>[p.id,Number(p.x.toFixed(6)),Number(p.y.toFixed(6)),String(p.tacticalTask||'')])}};
}
function draw(){
  if(!match)return;
