...
function collect(opened,start,duration=7){const {E,P,state}=opened,frames=[deep(E.snapshot(state.m))];state.mode='FULL_SKIP';let guard=0;while(!state.m.completed&&!state.pending&&state.m.time<start+duration&&guard++<2400){P.step(state,.05);frames.push(deep(E.snapshot(state.m)));P.maybeCheckpoint(state);if(state.pending)break;const recent=(state.m.events||[]).filter(e=>e.t>=start-.001);if(recent.some(e=>['GOAL','SAVE','CHIP_SAVE','SHOT_MISSED','CORNER','GOAL_KICK','THROW_IN','OFFSIDE'].includes(e.type))&&state.m.time>start+.75)break;}return{frames,pending:state.pending?deep(state.pending):null,snapshot:E.snapshot(state.m),actualEvents:(state.m.events||[]).filter(e=>e.t>=start-.001).map(deep)};}
function runCross(key,seed,opts={}){
  ...
  const start=m.time;br.executePass(m,winger,st,'CROSS',{running:true},'FINAL_MATCH_RARE_SCENE');
  const out=collect(opened,start,7);return{key,label:c.label,seed,boundary:b,entrySnapshot:deep(E.snapshot(m)),...out,forcedSetup:'CROSS_RELEASE_ONLY',futureOutcomePrecomputed:false};
}
function runBreakaway(key,seed,opts={}){
  ...
  m.possession='HOME';m.protagonistExplicitActionRequired=true;br.setControlled(m,st);st.controlledSince=m.time-1.0;st.nextThink=m.time;st.lockTargetUntil=0;st.action='HOLD_BALL';st.tacticalTask='HOLD_BALL';st.bodyAngle=0;st.faceTargetAngle=0;st.tx=clamp(st.x+3.2,3,101);st.ty=clamp(st.y+signed(seed,'carry-y',1.6),3,65);st.vx=2.8;st.vy=(st.ty-st.y)/1.2;state.mode='PLAYER_ALL';state.lastPauseAt=m.time-1.0;state.lastPauseControlledSince=-999;state.forceNextChoice=true;
  P.maybeCheckpoint(state);let guard=0;while(!state.pending&&guard++<14){P.step(state,.05);P.maybeCheckpoint(state);}
  const pending=state.pending?deep(state.pending):null;if(!pending)throw new Error('BREAKAWAY_CHOICE_NOT_FOUND');
  return{key,label:c.label,seed,boundary:b,entrySnapshot:deep(E.snapshot(m)),frames:[deep(E.snapshot(m))],pending,snapshot:E.snapshot(m),actualEvents:(m.events||[]).slice(-10).map(deep),forcedSetup:'BREAKAWAY_CONTEXT_AND_USER_DECISION_ONLY',futureOutcomePrecomputed:false};
}
function runGkShot(key,seed,opts={}){
  const c=SCENARIOS[key],b=boundary(key,seed),opened=A.seedMatch(b,{seed:`${seed}|ENTRY`,runtimeDir:runtimeDir(opts),explicitHeroChoiceRequired:true}),{E,state}=opened,m=state.m,br=E.choiceActionBridge(),shooter=br.playerById(m,'A-ST'),gk=br.playerById(m,'H-GK');
  ...
  m.possession='AWAY';m.protagonistExplicitActionRequired=true;br.setControlled(m,shooter);shooter.controlledSince=m.time-.9;shooter.nextThink=m.time+99;const start=m.time;br.executeShot(m,shooter,'FINAL_MATCH_RARE_SCENE',{releaseNow:true});const out=collect(opened,start,6);
  return{key,label:c.label,seed,boundary:b,entrySnapshot:deep(E.snapshot(m)),...out,forcedSetup:'SHOT_RELEASE_ONLY',futureOutcomePrecomputed:false};
}
function runSetPiece(key,seed,opts={}){const c=SCENARIOS[key],b=boundary(key,seed),out=A.runSetPieceWindow(b,{seed:`${seed}|SETPIECE`,runtimeDir:runtimeDir(opts),durationSeconds:10});return{key,label:c.label,seed,boundary:b,...out,forcedSetup:'DEAD_BALL_CONTEXT_ONLY',futureOutcomePrecomputed:false};}
...
function summary(result){const events=result.actualEvents||[],pending=result.pending||null,first=result.frames?.[0]||result.entrySnapshot,last=result.frames?.at?.(-1)||result.snapshot;return{key:result.key,label:result.label,seed:result.seed,forcedSetup:result.forcedSetup,futureOutcomePrecomputed:result.futureOutcomePrecomputed,hadChoice:!!pending,choiceKind:pending?.kind||null,optionIds:(pending?.options||[]).map(o=>o.id),events:events.map(e=>e.type),entry:{time:first?.time,ball:first?.ball?{mode:first.ball.mode,x:first.ball.x,y:first.ball.y,ownerId:first.ball.ownerId}:null},exit:{time:last?.time,ball:last?.ball?{mode:last.ball.mode,x:last.ball.x,y:last.ball.y,ownerId:last.ball.ownerId}:null}};}
